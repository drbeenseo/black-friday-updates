...
if (!function_exists('cmb2_render_switch')) {
    function cmb2_render_switch($field, $escaped_value, $object_id, $object_type, $field_type_object)
    {
        ...
        $label_off = (isset($field->args['label']) ? esc_attr($field->args['label']['off']) : 'Off');
            $switch .= '<input ' . $conditional_value . $conditional_id . ' type="radio" id="' . $field->args['id'] . '1" value="on"  ' . ($escaped_value == 'on' ? 'checked="checked"' : '') . ' name="' . esc_attr($field->args['_name']) . '" />
		<input ' . $conditional_value . $conditional_id . ' type="radio" id="' . $field->args['id'] . '2" value="off" ' . (($escaped_value == '' || $escaped_value == 'off') ? 'checked="checked"' : '') . ' name="' . esc_attr($field->args['_name']) . '" />
		<label for="' . $field->args['id'] . '1" class="cmb2-enable ' . ($escaped_value == 'on' ? 'selected' : '') . '"><span>' . $label_on . '</span></label>
		<label for="' . $field->args['id'] . '2" class="cmb2-disable ' . (($escaped_value == '' || $escaped_value == 'off') ? 'selected' : '') . '"><span>' . $label_off . '</span></label>';
