...
class KS_Giveaways_Admin
{
    ...
    public function input_show_ks_badge()
    {
        echo sprintf('<input type="checkbox" id="%s" name="%s" value="1"%s />',
            ...
            KS_GIVEAWAYS_OPTION_SHOW_KS_BADGE,
            get_option(KS_GIVEAWAYS_OPTION_SHOW_KS_BADGE) ? ' checked' : ''
        );
    }
...
EOF;
    ...
    public function save_contest($post_id)
    {
        ...
        $entries_per_youtube_subscribe = (int) $_POST['entries_per_youtube_subscribe'] ? (int) stripslashes_deep(sanitize_text_field($_POST['entries_per_youtube_subscribe'])) : 3;
        $entries_per_twitter_follow = (int) $_POST['entries_per_twitter_follow'] ? (int) stripslashes_deep(sanitize_text_field($_POST['entries_per_twitter_follow'])) : 3;
        $entries_per_facebook_like = (int) $_POST['entries_per_facebook_like'] ? (int) stripslashes_deep(sanitize_text_field($_POST['entries_per_facebook_like'])) : 3;
        ...
        update_post_meta($post_id, '_entries_per_youtube_subscribe', $entries_per_youtube_subscribe);
        update_post_meta($post_id, '_entries_per_twitter_follow', $entries_per_twitter_follow);
        update_post_meta($post_id, '_entries_per_facebook_like', $entries_per_facebook_like);
        ...
        $keys = array(
            ...
            KS_GIVEAWAYS_OPTION_CONVERTKI_FORM_ID,
