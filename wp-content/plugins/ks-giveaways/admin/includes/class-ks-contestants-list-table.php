...
class KS_Contestants_List_Table extends WP_List_Table
{
    ...
    public function extra_tablenav($which)
    {
        if ($which == 'top') {
            ...
            echo '<p><a href="'.admin_url('admin.php?page=ks-giveaways&action=contestants&id='.$this->contest_id).'&noheader=true&downloadcsv=true" class="button button-primary" style="text-align:center">Download CSV</a></p>';
            echo '<p><input type="submit" style="display: inline-block;" name="bulkremove" class="button button-secondary" onclick="return confirm(\'Remove all unconfirmed contestants?\');" value="Remove unconfirmed">';
            echo '<input type="submit" style="display: inline-block; margin: 0 0 0 8px;" name="bulkresend" class="button button-secondary" onclick="return confirm(\'Resend confirmation email to all unconfirmed contestants?\');" value="Bulk resend"></p>';
            echo '</div>';
            echo '</form>';
        }
    }
