...
class KS_Giveaways
{
    ...
    private function __construct()
    {
        ...
        add_action('wp_ajax_ajax_ks_giveaways_ajax_award_link_clicked', array($this, 'ajax_ks_giveaways_ajax_award_link_clicked'));
        add_action('wp_ajax_nopriv_ajax_ks_giveaways_ajax_award_link_clicked', array($this, 'ajax_ks_giveaways_ajax_award_link_clicked'));
...
EOF
     ...
     */
    public function ajax_ks_giveaways_ajax_award_link_clicked()
    {
        global $wpdb;
        if (isset($_POST['giveaway_id']) && isset($_POST['contestant_id']) && isset($_POST['link_clicked'])) {
            $post = get_post($_POST['giveaway_id']);
            ...
            if (get_post_type($post->ID) === KS_GIVEAWAYS_POST_TYPE) {
                ...
                if ($_POST['link_clicked'] === 'twitter') {
                    $number_of_entries =  get_post_meta($post->ID, '_entries_per_twitter_follow', true);

                } elseif($_POST['link_clicked'] === 'facebook') {
                    $number_of_entries =  get_post_meta($post->ID, '_entries_per_facebook_like', true);

                } else {
                    $number_of_entries =  get_post_meta($post->ID, '_entries_per_youtube_subscribe', true);
                }

                $existing_link_entries =  KS_Entry_DB::get_link_entry_count($contestant->ID, $_POST['link_clicked']);

                // start transaction to avoid 0 entries
                $wpdb->query('START TRANSACTION');

                if (!KS_Entry_DB::add($contestant->ID, NULL, $number_of_entries - $existing_link_entries, $_POST['link_clicked'])) {
                    $wpdb->query('ROLLBACK');
                    return false;
                }
