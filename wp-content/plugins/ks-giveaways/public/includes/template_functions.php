...
function ks_giveaways_get_entries_per_youtube_subscribe()
{
  return KS_Helper::get_entries_per_youtube_subscribe();
}

function ks_giveaways_show_youtube_section()
{
  $url = ks_giveaways_get_youtube_channel_url();
  $available_entries = ks_giveaways_get_entries_per_youtube_subscribe();
  $current_entries = ks_giveaways_get_link_entry_count(ks_giveaways_get_youtube_channel_url());

  return $url && $available_entries && $current_entries < $available_entries;
}

function ks_giveaways_get_entries_per_twitter_follow()
{
  return KS_Helper::get_entries_per_twitter_follow();
}

function ks_givewaways_show_twitter_follow()
{
  $available_entries = ks_giveaways_get_entries_per_twitter_follow();
  $current_entries = ks_giveaways_get_link_entry_count('twitter');

  return get_option(KS_GIVEAWAYS_OPTION_TWITTER_VIA) && $available_entries && $current_entries < $available_entries;
}

function ks_giveaways_get_entries_per_facebook_like()
{
  return KS_Helper::get_entries_per_facebook_like();
}

function ks_giveaways_show_facebook_like()
{
  $available_entries = ks_giveaways_get_entries_per_facebook_like();
  $current_entries = ks_giveaways_get_link_entry_count('facebook');

  return get_option(KS_GIVEAWAYS_OPTION_FACEBOOK_PAGE) && $available_entries && $current_entries < $available_entries;
}
