...
jQuery(document).ready(function($) {
  ...
  window.award_link_clicked = function(url, callback) {
    var data = {
                'action'       : 'ajax_ks_giveaways_ajax_award_link_clicked',
                'giveaway_id'  : ks_giveaways_globals.giveaway_id,
                'contestant_id': ks_giveaways_globals.contestant_id,
                'link_clicked' : url
              };

    $.post(ks_giveaways_globals.ajax_url, data, function(response) {
      if (response.hasOwnProperty('total_entries')) {
        $('#youtube-subscribe-section').hide();
        // Update entry count
        $('#total-entries').text(response.total_entries + ' ' + (response.total_entries > 1 ? 'entries' : 'entry'));
      }

      if (typeof callback === 'function') {
        callback(response);
        renumber_extra_entry_sections();
      }
    }, 'json');
  };

  window.renumber_extra_entry_sections = function() {
    $('.youtube-subscribe-section, .twitter-follow-section, .facebook-like-section').each(function(index, element) {
      if (element) {
  ...
  $('.youtube-subscribe-link').on('click', function(e) {
    award_link_clicked($(this).data('originalUrl'), function(response) {
      $('#youtube-subscribe-section, .youtube-subscribe-section').remove();
    });
  });

  twttr.ready(function() {
    twttr.events.bind('follow', function(event) {
      award_link_clicked('twitter', function(response) {
        $('.twitter-follow-section').remove();
      });
    });
  });
