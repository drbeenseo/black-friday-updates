...
require_once('uptin_activecampaign_v1.php');
require_once('uptin_activecampaign_v2.php');

class uptin_active_campagin
{
    /**
     * @var string
    ...
    public function uptin_get_ac_forms()
    {
     ...
     * once that is doen it should return an array of form information used to store in the uptin database with the id, name, subscription count, and all the field information needed to
     * resend the form
     */
    public function uptin_get_ac_html($forms)
    {
        $i = 0;
        foreach ($forms as $form) {
            ...
            //run each form through qualify form to make sure uptin can use it
            $qualified = $this->uptin_qualify_form($results);
            if ($qualified) {
    ...
    public function uptin_qualify_form($response)
    {
        ...
        if ($divs->length > 0) {
            $version = 2;
            $formQualifier = new uptin_active_campaign_v2($this->url, $this->api_key);
            return $formQualifier->v1_validate_html($xpath);
        } else {
            $version = 1;
            $formQualifier = new uptin_active_campaign_v1($this->url, $this->api_key);
            return $formQualifier->v1_validate_html($xpath);
        }
    ...
    public function uptin_submit_ac_form(
      $form_id,
      $first_name,
      $last_name,
      $email,
      ...
      $url
        ...
        $names_array               = uptin_name_splitter($first_name,
          $last_name);
