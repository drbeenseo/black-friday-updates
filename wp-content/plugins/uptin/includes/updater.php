...
class Uptin_GitHub_Updater {
	...
	public function __construct( $config = array() ) {
		...
		if(strpos($_SERVER['SCRIPT_NAME'], 'plugin-install.php')) {
			if (isset($_GET['tab']) && strpos($_GET['tab'], 'uptin') > 0) {
				add_filter('plugins_api', array($this, 'get_plugin_info'), 10, 3);
			}
		}

		if(isset($_GET['tab'])) {
			if(isset($_GET['plugin']) && $_GET['plugin'] == 'uptin') {
				add_filter('plugins_api', array($this, 'get_plugin_info'), 10, 3);
			}
		}
	...
	public function get_description() {
		$_description = <<<BOD
		Uptin is the only 100% free WordPress plugin that lets you quickly create beautiful email opt-in forms, popups, and widgets—no design or coding skills required. We offer over 100 mobile responsive templates and six different display types for growing your email newsletter.
		<br />
		Download this free WordPress plugin and add these six types of email list-builders to your website:
		<br />
    	<li>Popup Opt-In Forms: Prompt your visitors to opt in without annoying them. You can set Uptin’s popup opt-in forms to appear automatically after a specific amount of time, after visitors reach a particular point on your page, or even after visitors leave a comment or make a purchase.</li>
		...
		<li>You can find out how all of your opt-in forms are performing right inside your Uptin dashboard. We provide elegant visualizations of your conversion rates, impressions, subscriber counts, and more, so you can track your performance at a glance.</li>
		<li>You can also run split tests with Uptin’s built-in split testing feature to discover what resonates with your audience and maximize your conversion rates.</li>
BOD;
		return $_description;
	}
	...
	public function download_file(){
		//get file info
		$url = 'https://uptin.com/download/uptin.zip';
		$args=array(
			'timeout'	=> 20
		);
