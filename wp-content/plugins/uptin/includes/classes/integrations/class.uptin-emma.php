...
class uptin_emma extends EWD_Uptin
{
	...
	public function draw_emma_form($form_fields, $service, $field_values)
	{
		$form_fields .= sprintf( '
			...
			__( 'Public API Key', 'uptin' ),
			__( 'Private API key', 'uptin' ),
			__( 'Account ID', 'uptin' ),
	...
	function get_emma_groups( $public_key, $private_key, $account_id, $name ) {
		if ( ! class_exists( 'Emma_Uptin' ) ) {
			require_once( EWD_UPTIN_PLUGIN_DIR . 'subscription/emma/Emma.php' );
		}

		$emma = new Emma_Uptin( $account_id, $public_key, $private_key, false ); //true set for debug
		try {
