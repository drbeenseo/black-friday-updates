...
class uptin_feedblitz extends EWD_Uptin
{
		...
		if ( ! is_wp_error( $theme_request ) && $response_code == 200 ) {
			...
			if ( ! empty( $theme_response ) ) {
				if ( 'ok' == $theme_response['rsp']['@attributes']['stat'] ) {
					...
					$error_message = isset( $theme_response['rsp']['err']['@attributes']['msg'] ) ? $theme_response['rsp']['err']['@attributes']['msg'] : __( 'Unknown error', 'uptin' );
				}
