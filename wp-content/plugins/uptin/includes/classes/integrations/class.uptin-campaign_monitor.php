...
class uptin_campaign_monitor extends EWD_Uptin
{
	...
	function get_campaign_monitor_lists( $api_key, $name ) {
		require_once( EWD_UPTIN_PLUGIN_DIR . 'subscription/createsend-php-4.0.2/csrest_clients.php' );
		require_once( EWD_UPTIN_PLUGIN_DIR . 'subscription/createsend-php-4.0.2/csrest_lists.php' );
		...
		if ( '200' == $httpCode ) {
			...
			if ( '401' == $httpCode ) {
				$error_message = __( 'invalid API key', 'uptin' );
			} else {
	...
	function subscribe_campaign_monitor( $api_key, $email, $list_id, $name = '' ) {
		require_once( EWD_UPTIN_PLUGIN_DIR . 'subscription/createsend-php-4.0.2/csrest_subscribers.php' );
		$auth          = array(
			'api_key' => $api_key,
		);
		...
		if ( $is_subscribed->was_successful() ) {
		  //switched error message to success as they do not need to know they were already subscribed when the submit the opt in
			//$error_message = __( 'Already subscribed', 'uptin' );
		  $error_message = 'success';
		} else {
