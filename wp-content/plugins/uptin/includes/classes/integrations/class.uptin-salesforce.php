...
class uptin_salesforce extends EWD_Uptin{
	...
	public function draw_salesforce_form($form_fields, $service, $field_values = ''){
		...
		$form_fields .= sprintf('
			...
			__('Instance Number', 'uptin'),#8
			__('Salesforce version #', 'uptin'),#9
			__('Consumer key', 'uptin'),#10
			__('Consumer secret', 'uptin'),#11
			__('Salesforce username', 'uptin'),#12
			__('Salesforce password', 'uptin'),#13
			__('Secuirty token', 'uptin'),#14
	...
	public function get_salesforce_campagins($url, $version, $client_key, $client_secret, $username_sf, $password_sf, $token, $name){
		$error_message='';
		if(!class_exists('Uptin_SalesforceAPI')) {
			require_once(EWD_UPTIN_PLUGIN_DIR . 'subscription/salesforce/SalesforceAPI.php');
		}
		...
		$salesforce = new Uptin_SalesforceAPI($url, $version, $client_key, $client_secret);
	...
	public function subscribe_salesforce($url, $version, $client_key, $client_secret, $username_sf, $password_sf, $token, $name, $last_name, $email, $list_id){
		...
		if($campaign_member->success == '1'){
			...
			$error_message = __('Lead could not be added to campaign', 'uptin');
		}
