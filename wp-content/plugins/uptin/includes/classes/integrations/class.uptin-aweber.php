...
class uptin_aweber extends EWD_Uptin
{
	...
	public function draw_aweber_form($form_fields, $service, $field_values)
	{
		...
		$form_fields .= sprintf( '
			...
			sprintf(
				__( 'Step 1: <a href="%1$s" target="_blank">Generate authorization code</a><br/>', 'uptin' ),
			...
			sprintf( '
				...
				__( 'Step 2: Paste in the authorization code and click "Authorize" button: ', 'uptin' ),
	...
	function get_aweber_lists( $api_key, $name ) {
		...
		if ( 'success' === $error_message ) {
			if ( ! class_exists( 'AWeberAPI' ) ) {
				require_once( EWD_UPTIN_PLUGIN_DIR . 'subscription/aweber/aweber_api.php' );
			}
	...
	function subscribe_aweber( $list_id, $account_name, $email, $name = '' ) {
		...
		if ( ! $account ) {
			$error_message = __( 'Aweber: Wrong configuration data', 'uptin' );
		}
	...
	function aweber_authorization( $api_key, $name ) {
		...
		try {
			...
			if ( ! ( is_array( $auth ) && 4 === count( $auth ) ) ) {
				$error_message = __( 'Authorization code is invalid. Try regenerating it and paste in the new code.', 'uptin' );
			} else {
				$error_message = 'success';
			...
			$error_message = sprintf(
				...
				esc_html__( 'AWeberAPIException.', 'uptin' ),
				esc_html__( 'Type', 'uptin' ),
				esc_html__( 'Message', 'uptin' ),
				esc_html__( 'Documentation', 'uptin' )
			);
		}
