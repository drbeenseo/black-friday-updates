...
class uptin_madmimi extends EWD_Uptin
{
	...
	public function get_madmimi_lists( $username, $api_key, $name ) {
		...
		if ( ! is_wp_error( $theme_request ) && $response_code == 200 ) {
			...
			if ( ! empty( $theme_response ) ) {
				...
				$error_message = __( 'Please make sure you have at least 1 list in your account and try again', 'uptin' );
			}
		} else {
				...
				if ( ! is_wp_error( $theme_request ) && $response_code == 200 ) {
					...
					if ( is_wp_error( $theme_request ) ) {
						...
						switch ( $response_code ) {
							case '401' :
								$error_message = __( 'Invalid Username or API key', 'uptin' );
								break;
							case '400' :
