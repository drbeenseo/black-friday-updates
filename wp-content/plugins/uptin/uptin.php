...
bypass registration
*/
update_option('rad_uptin_activated', 'uptin_activated');
update_option('rad_uptin_activated_code', '100');

require 'uptin_wp_updater/plugin-update-checker.php';
$myUpdateChecker = PucFactory::buildUpdateChecker(
    'http://uptin.s3.amazonaws.com/update_uptin.json',
    __FILE__,
    'uptin'
);
...
define( 'EWD_UPTIN_PLUGIN_DIR', trailingslashit( dirname( __FILE__ ) ) );
define( 'EWD_UPTIN_PLUGIN_URI', plugins_url( '', __FILE__ ) );

if ( ! class_exists( 'EWD_Dash' ) ) {
	require_once( EWD_UPTIN_PLUGIN_DIR . 'dashboard/dashboard.php' );
}

if ( ! class_exists( 'uptin_rapidbar' ) ) {
	require_once( EWD_UPTIN_PLUGIN_DIR . 'includes/ext/uptin_rapidbar/class.uptin_rapidbar.php' );
}

// require_once('includes/updater.php');
require_once('includes/uptin_functions.php');

class EWD_Uptin extends EWD_Dash {
	var $plugin_version = '1.0.6';
	var $db_version = '1.0';
	var $_options_pagename = 'rad_uptin_options';
	var $menu_page;
	var $protocol;
	var $privacy_url = 'http://www.uptin.com/privacy';
	var $tou_url = 'http://www.uptin.com/tou';
	...
	function __construct() {
		...
		/*if ( isset( self::$_this ) ) {
			wp_die( sprintf( __( '%s is a singleton class and you cannot create a second instance.', 'uptin' ),
					get_class( $this ) )
			);
		}*/
		global $pagenow;
		...
		// add_action('admin_init', array( $this,'rapidologly_update' ) ); // markramos

		add_filter( 'rad_uptin_import_sub_array', array( $this, 'import_settings' ) );
		add_filter( 'rad_uptin_import_array', array( $this, 'import_filter' ) );
		add_filter( 'rad_uptin_export_exclude', array( $this, 'filter_export_settings' ) );
		add_filter( 'rad_uptin_save_button_class', array( $this, 'save_btn_class' ) );


		// generate home tab in dashboard
		add_action( 'rad_uptin_after_header_options', array( $this, 'generate_home_tab' ) );

		add_action( 'rad_uptin_after_main_options', array( $this, 'generate_premade_templates' ) );

		add_action( 'rad_uptin_after_save_button', array( $this, 'add_next_button' ) );
		do_action('uptin_ext_init');
		...
		$dashboard_args = array(
			...
			'rad_dashboard_plugin_name'       => 'uptin',
			'rad_dashboard_save_button_text'  => __( 'Save & Exit', 'uptin' ),
			'rad_dashboard_plugin_class_name' => 'rad_uptin',
			'rad_dashboard_options_path'      => EWD_UPTIN_PLUGIN_DIR . 'dashboard/includes/options.php',
		...
		add_action( 'wp_ajax_rad_uptin_save_settings', array( $this, 'uptin_save_settings' ) );

		add_action( 'admin_enqueue_scripts', array( $this, 'register_scripts' ) );

		add_action( 'wp_enqueue_scripts', array( $this, 'load_scripts_styles' ) );

		add_action( 'wp_ajax_uptin_reset_options_page', array( $this, 'uptin_reset_options_page' ) );

		add_action( 'wp_ajax_uptin_remove_optin', array( $this, 'remove_optin' ) );

		add_action( 'wp_ajax_uptin_duplicate_optin', array( $this, 'duplicate_optin' ) );

		add_action( 'wp_ajax_uptin_add_variant', array( $this, 'add_variant' ) );

		add_action( 'wp_ajax_uptin_home_tab_tables', array( $this, 'home_tab_tables' ) );

		add_action( 'wp_ajax_uptin_toggle_optin_status', array( $this, 'toggle_optin_status' ) );

		add_action( 'wp_ajax_uptin_authorize_account', array( $this, 'authorize_account' ) );

		add_action( 'wp_ajax_uptin_reset_accounts_table', array( $this, 'reset_accounts_table' ) );

		add_action( 'wp_ajax_uptin_generate_mailing_lists', array( $this, 'generate_mailing_lists' ) );

		add_action( 'wp_ajax_uptin_generate_new_account_fields', array( $this, 'generate_new_account_fields' ) );

		add_action( 'wp_ajax_uptin_generate_accounts_list', array( $this, 'generate_accounts_list' ) );

		add_action( 'wp_ajax_uptin_generate_current_lists', array( $this, 'generate_current_lists' ) );

		add_action( 'wp_ajax_uptin_generate_edit_account_page', array( $this, 'generate_edit_account_page' ) );

		add_action( 'wp_ajax_uptin_save_account_tab', array( $this, 'save_account_tab' ) );

		add_action( 'wp_ajax_uptin_ab_test_actions', array( $this, 'ab_test_actions' ) );

		add_action( 'wp_ajax_uptin_get_stats_graph_ajax', array( $this, 'get_stats_graph_ajax' ) );

		add_action( 'wp_ajax_uptin_refresh_optins_stats_table', array( $this, 'refresh_optins_stats_table' ) );

		add_action( 'wp_ajax_uptin_reset_stats', array( $this, 'reset_stats' ) );

		add_action( 'wp_ajax_uptin_pick_winner_optin', array( $this, 'pick_winner_optin' ) );

		add_action( 'wp_ajax_uptin_clear_stats', array( $this, 'clear_stats' ) );
	    add_action( 'wp_ajax_uptin_clear_stats_single_optin', array( $this, 'clear_stats_single_optin' ) );


		add_action( 'wp_ajax_uptin_get_premade_values', array( $this, 'get_premade_values' ) );
		add_action( 'wp_ajax_uptin_generate_template_filter', array( $this, 'generate_template_filter' ) );
		add_action( 'wp_ajax_uptin_generate_premade_grid', array( $this, 'generate_premade_grid' ) );

		add_action( 'wp_ajax_uptin_display_preview', array( $this, 'display_preview' ) );

		add_action( 'wp_ajax_uptin_handle_stats_adding', array( $this, 'handle_stats_adding' ) );
		add_action( 'wp_ajax_nopriv_uptin_handle_stats_adding', array( $this, 'handle_stats_adding' ) );

		add_action( 'wp_ajax_uptin_subscribe', array( $this, 'subscribe' ) );
		add_action( 'wp_ajax_nopriv_uptin_subscribe', array( $this, 'subscribe' ) );

		add_action( 'widgets_init', array( $this, 'register_widget' ) );

		add_action( 'after_setup_theme', array( $this, 'register_image_sizes' ) );

		add_shortcode( 'rad_uptin_inline', array( $this, 'display_inline_shortcode' ) );
		add_shortcode( 'rad_uptin_locked', array( $this, 'display_locked_shortcode' ) );

		add_filter( 'body_class', array( $this, 'add_body_class' ) );
		register_activation_hook( __FILE__, 'rapid_version_check' );

		if($pagenow == 'plugins.php' || isset($_GET['page']) && $_GET['page']=='rad_uptin_options'){
			add_action( 'admin_notices', 'rapid_version_check' );
		}

		if ( ! wp_next_scheduled( 'uptin_update_source_check' ) ) {
		  wp_schedule_event( time(), 'daily', 'uptin_update_source_check' );
		}

		register_activation_hook( __FILE__, array( $this, 'activate_plugin' ) );
		register_deactivation_hook( __FILE__, array( $this, 'deactivate_plugin' ) );
		add_action( 'uptin_lists_auto_refresh', array( $this, 'perform_auto_refresh' ) );
		add_action( 'uptin_stats_auto_refresh', array( $this, 'perform_stats_refresh' ) );
	  	add_action( 'uptin_update_source_check', array($this, 'uptin_update_source' ));
	...
	function activate_plugin() {
		...
		wp_schedule_event( time(), 'daily', 'uptin_lists_auto_refresh' );
	...
	function deactivate_plugin() {
		...
		wp_clear_scheduled_hook( 'uptin_lists_auto_refresh' );
		wp_clear_scheduled_hook( 'uptin_stats_auto_refresh' );
	}
	...
	function uptin_update_source() {
	  $update = wp_remote_get('https://uptin.com/download/wp_update.json?version=4');
	  $update = json_decode($update['body']);
	  update_option( 'uptin_update_source', $update->wordpress_update );
	}
	...
	// 	$update = get_option( 'uptin_update_source', false );
	...
	// 				'zip_url' => 'https://uptin.com/download/uptin.zip', // the zip url of the github repo
	// 				'release_url' => 'https://api.github.com/repos/leadpages/uptin-plugin/releases',
	// 				'api_url' => 'https://api.github.com/repos/leadpages/uptin-plugin', // the github API url of your github repo
	// 				'raw_url' => 'https://raw.github.com/leadpages/uptin-plugin/master', // the github raw url of your github repo
	// 				'github_url' => 'https://github.com/leadpages/uptin-plugin', // the github url of your github repo
	...
	// 			);
	// 			new Uptin_GitHub_Updater($config);
	// 		}
	// 	}
	// }
	...
	function add_menu_link() {
		...
		add_submenu_page( 'rad_uptin_options', __( 'Uptin', 'uptin' ), __( 'Tools & Resources', 'uptin' ), 'manage_options', 'rad_uptin_options' ); /* commented by mark ramos, to remove the this from menu */
		add_submenu_page( 'rad_uptin_options', __( 'Optin Forms', 'uptin' ), __( 'Optin Forms', 'uptin' ), 'manage_options', 'admin.php?page=rad_uptin_options#tab_rad_dashboard_tab_content_header_home' );
		add_submenu_page( 'rad_uptin_options', __( 'Email Accounts', 'uptin' ), __( 'Email Accounts', 'uptin' ), 'manage_options', 'admin.php?page=rad_uptin_options#tab_rad_dashboard_tab_content_header_accounts' );
		add_submenu_page( 'rad_uptin_options', __( 'Statistics', 'uptin' ), __( 'Statistics', 'uptin' ), 'manage_options', 'admin.php?page=rad_uptin_options#tab_rad_dashboard_tab_content_header_stats' );
		add_submenu_page( 'rad_uptin_options', __( 'Import & Export', 'uptin' ), __( 'Import & Export', 'uptin' ), 'manage_options', 'admin.php?page=rad_uptin_options#tab_rad_dashboard_tab_content_header_importexport' );
	}

	function add_body_class( $body_class ) {
		$body_class[] = 'rad_uptin';
	...
	function add_localization() {
		load_plugin_textdomain( 'uptin', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	}

	// Add settings link on plugin page
	function add_settings_link( $links ) {
		$settings_link = sprintf( '<a href="admin.php?page=rad_uptin_options">%1$s</a>', __( 'Settings', 'uptin' ) );
	...
	function options_page() {
		EWD_Uptin::generate_options_page( $this->generate_optin_id() );
	}
	...
	function uptin_save_settings() {
		EWD_Uptin::dashboard_save_settings();
	}
	...
	/**
	 *
	 * Adds the "Next" button into the Uptin dashboard via EWD_Dash action.
	...
	function add_next_button() {
		...
		printf( '
			...
			__( 'Next: Display Settings', 'uptin' )
		);
		...
		printf( '
			...
			__( 'Generate Shortcode', 'uptin' )
		);
	}

	/**
	 * Retrieves the Uptin options from DB and makes it available outside the class
	 * @return array
	 */
	public static function get_uptin_options() {
		return get_option( 'rad_uptin_options' ) ? get_option( 'rad_uptin_options' ) : array();
	}

	/**
	 * Updates the Uptin options outside the class
	 * @return void
	 */
	public static function update_uptin_options( $update_array ) {
		$dashboard_options = EWD_Uptin::get_uptin_options();

		$updated_options = array_merge( $dashboard_options, $update_array );
		update_option( 'rad_uptin_options', $updated_options );
	}
	 ...
	 * Filter is used in EWD_Dash class
	 * @return array
	 */
	...
	function add_mce_button( $plugin_array ) {
		...
		wp_enqueue_style( 'uptin-shortcodes', EWD_UPTIN_PLUGIN_URI . '/css/tinymcebutton.css', array(), $this->plugin_version );
		$plugin_array['uptin'] = EWD_UPTIN_PLUGIN_URI . '/js/uptin-mce-buttons.js';
	...
	function register_mce_button( $buttons ) {
		...
		array_push( $buttons, 'uptin_button' );
	...
	function tiny_mce_vars() {
		$options_array = EWD_Uptin::get_uptin_options();
		$locked_array  = array();
		$inline_array  = array();
		$onclick_array = array();
		...
		if ( empty( $locked_array ) ) {
			$locked_array = array(
				'empty' => __( 'No optins available', 'uptin' ),
			);
		}
		...
		<script type='text/javascript'>
			var uptin = {
				...
				'uptin_tooltip': '<?php _e( "insert uptin Opt-In", "uptin" ); ?>',
				'inline_text': '<?php _e( "Inline Opt-In", "uptin" ); ?>',
				'locked_text': '<?php _e( "Locked Content Opt-In", "uptin" ); ?>'
			}
		</script>
		<!-- TinyMCE Shortcode Plugin -->
		<?php
	...
	function db_install() {
		...
		$table_name = $wpdb->prefix . 'rad_uptin_stats';
		...
		EWD_Uptin::update_option( $db_version );
	}

	function register_image_sizes() {
		add_image_size( 'uptin_image', 610 );
	}

	/**
	 * Generates the Uptin's Home, Stats, Accounts tabs. Hooked to Dashboard class
	 */
		...
		switch ( $option['type'] ) {
			case 'home' :
				printf( '
					...
					esc_html__( 'new optin', 'uptin' ),
					esc_html__( 'Active Opt-Ins', 'uptin' )
				);
				printf( '
					...
					esc_html__( 'select optin type to begin', 'uptin' ),
					esc_html__( 'pop up', 'uptin' ),
					esc_html__( 'fly in', 'uptin' ),
					esc_html__( 'below post', 'uptin' ),
					esc_html__( 'inline', 'uptin' ),
					esc_html__( 'locked content', 'uptin' ),
					esc_html__( 'widget', 'uptin' ),
					esc_html__( 'bar', 'uptin' )
				);
			...
			case 'account' :
				printf( '
					...
					esc_html__( 'new account', 'uptin' ),
					esc_html__( 'My Accounts', 'uptin' )
				);
			...
			case 'stats' :
				printf( '
					<div class="rad_dashboard_row rad_dashboard_stats_row">
						<h1>%1$s</h1>
						<div class="rad_uptin_stats_controls">
							<button class="rad_dashboard_icon rad_uptin_clear_stats">%2$s</button>
							<span class="rad_dashboard_confirmation">%4$s</span>
							<button class="rad_dashboard_icon rad_uptin_refresh_stats">%3$s</button>
						</div>
					</div>
					<span class="rad_uptin_stats_spinner"></span>
					<div class="rad_dashboard_stats_contents"></div>',
					esc_html( $option['title'] ),
					esc_html__( 'Clear Stats', 'uptin' ),
					esc_html__( 'Refresh Stats', 'uptin' ),
					sprintf(
						'%1$s<span class="rad_dashboard_confirm_stats">%2$s</span><span class="rad_dashboard_cancel_delete">%3$s</span>',
						esc_html__( 'Remove all the stats data?', 'uptin' ),
						esc_html__( 'Yes', 'uptin' ),
						esc_html__( 'No', 'uptin' )
					)
				);
				break;
			...
			case 'support'	:
				include_once(EWD_UPTIN_PLUGIN_DIR.'includes/static_content/marketing.php');
				break;
		}
	}
	...
	function generate_premade_templates( $option ) {
		switch ( $option['type'] ) {
			case 'premade_templates' :
				echo '<div class="rad_uptin_premade_grid"><span class="spinner rad_uptin_premade_spinner"></span></div>';
				break;
			case 'preview_optin' :
				printf( '
	...
	function generate_template_filter(){
		...
		$filter_path = EWD_UPTIN_PLUGIN_URI.'/images';
		$isRapidBar     = '';
		$isRedirect     = '';
  ...
  <circle transform="translate(80 0)" cx="0" cy="14" r="0">
    <animate attributeName="r" values="0; 20; 0; 0" dur="1.2s" repeatCount="indefinite" begin="0.6"
      keytimes="0;0.2;0.7;1" keySplines="0.2 0.2 0.4 0.8;0.2 0.6 0.4 0.8;0.2 0.6 0.4 0.8" calcMode="spline" />
		...
		<div class="rad_uptin_premade_grid"></div>
SOL;
		die($output);
	}
	...
	function generate_premade_grid() {
		...
		$filter_path = EWD_UPTIN_PLUGIN_URI.'/images';
		if($isRapidBar == 'true'){
			require_once(EWD_UPTIN_PLUGIN_DIR . 'includes/ext/uptin_rapidbar/layouts/'.$layoutFolder.'/premade-layouts.php');
			$imgpath = EWD_UPTIN_PLUGIN_URI . '/includes/ext/uptin_rapidbar/layouts/'.$layoutFolder.'/images/thumb_';
		  	$rapidBarClass = ' rapidbar_layouts';
		}else {
			require_once(EWD_UPTIN_PLUGIN_DIR . 'includes/premade-layouts.php');
			$imgpath = EWD_UPTIN_PLUGIN_URI . '/images/thumb_';
		}
		...
		if ( isset( $select_layouts ) ) {
			...
			$output .= '<div class="rad_uptin_premade_grid'.$rapidBarClass.'">';

			foreach ( $select_layouts as $layout_id => $layout_options ) {
				$output .= sprintf( '
					<div class="rad_uptin_premade_item%2$s rad_uptin_premade_id_%1$s" data-layout="%1$s">
						<div class="rad_uptin_premade_item_inner">
							<img src="%3$s" alt="" />
						</div>
					</div>',
					esc_attr( $layout_id ),
					0 == $i ? ' rad_uptin_layout_selected' : '',
		...
		$output = sprintf( '
			<div class="rad_dashboard_stats_contents rad_dashboard_stats_ready">
				...
				<div class="rad_dashboard_optins_stats rad_dashboard_lists_stats_graph">
					<div class="rad_uptin_graph_header">
						<h3>%6$s</h3>
						<div class="rad_uptin_graph_controls">
							<a href="#" class="rad_uptin_graph_button rad_uptin_active_button" data-period="30">%7$s</a>
							<a href="#" class="rad_uptin_graph_button" data-period="12">%8$s</a>
							<select class="rad_uptin_graph_select_list">%9$s</select>
						</div>
					</div>
					%5$s
				</div>
			...
			esc_html__( 'New sign ups', 'uptin' ),
			esc_html__( 'Last 30 days', 'uptin' ),
			esc_html__( 'Last 12 month', 'uptin' ),
	...
	function reset_stats() {
		wp_verify_nonce( $_POST['uptin_stats_nonce'], 'uptin_stats' );

		$output = $this->generate_stats_tab();
		update_option( 'rad_uptin_stats_cache', $output );


		if ( ! wp_get_schedule( 'uptin_stats_auto_refresh' ) ) {
			wp_schedule_event( time(), 'daily', 'uptin_stats_auto_refresh' );
		}
	...
	function perform_stats_refresh() {
		...
		update_option( 'rad_uptin_stats_cache', $fresh_stats );
	}
  ...
  function clear_stats_single_optin() {
	wp_verify_nonce( $_POST['uptin_stats_nonce'], 'uptin_stats' );
	delete_option( 'rad_uptin_stats_cache' );
	global $wpdb;
	...
	function generate_all_lists_select() {
		$options_array = EWD_Uptin::get_uptin_options();
		$output        = sprintf( '<option value="all">%1$s</option>', __( 'All lists', 'uptin' ) );
	...
	function generate_all_time_stats( $empty_stats = false ) {
		...
		$ouptut = sprintf(
			...
			$conversion_rate . '%',
			__( 'Conversion Rate', 'uptin' ),
			$all_subscribers,
			__( 'Subscribers', 'uptin' ),
			$growth_rate,
			__( 'Subscriber Growth', 'uptin' ),
			__( 'week', 'uptin' )
		);
				...
				if ( 0 === $optins_count ) {
					if ( true == $include_header ) {
						$output .= sprintf(
							...
							__( 'Opt-In Form', 'uptin' ),
							__( 'Views', 'uptin' ),
							__( 'Opt-Ins', 'uptin' ),
							__( 'Conversion Rate', 'uptin' ),
						    __('Clear Stats', 'uptin')
						);
					}
		...
		if ( ! empty( $con_by_pages ) ) {
			...
			if ( ! empty( $rate_by_pages_sorted ) ) {
				...
				$output = sprintf(
					...
					__( 'Highest converting pages', 'uptin' ),
					__( 'Conversion rate', 'uptin' ),
					$table_contents
				);
			}
							...
							if ( 0 === $optins_count ) {
								if ( true == $include_header ) {
									$output .= sprintf(
										...
										esc_html__( 'My Lists', 'uptin' ),
										esc_html__( 'Provider', 'uptin' ),
										esc_html__( 'Subscribers', 'uptin' ),
										esc_html__( 'Growth Rate', 'uptin' )
									);
								}
		...
		if ( ! empty( $unsorted_array ) ) {
			...
			foreach ( $sorted_array as $single_list ) {
				$output .= sprintf(
					...
					'ontraport' == $single_list['service'] ? esc_html__( 'n/a', 'uptin' ) : esc_html( $single_list['count'] ),
					esc_html( $single_list['growth'] ),
					esc_html__( 'week', 'uptin' )
				);
			}
		}
	...
	function generate_stats_graph_output( $period, $day_or_month, $data ) {
		$result = '<div id="uptin_line_chart" class="rad_dashboard_lists_stats_graph_container"></div>';
		$bars_count = 0;
		for ( $i = 1; $i <= $period; $i ++ ) {
		...
		$result .= sprintf(
			...
			sprintf(
				...
				esc_html__( 'New Signups', 'uptin' )
			),
			sprintf(
				'%1$s %2$s %3$s',
				esc_html( $per_day ),
				esc_html__( 'Per', 'uptin' ),
				'day' == $day_or_month ? esc_html__( 'Day', 'uptin' ) : esc_html__( 'Month', 'uptin' )
			)
		);
	...
	function get_stats_graph_ajax() {
		wp_verify_nonce( $_POST['uptin_stats_nonce'], 'uptin_stats' );
		$list_id = ! empty( $_POST['uptin_list'] ) ? sanitize_text_field( $_POST['uptin_list'] ) : '';
		$period  = ! empty( $_POST['uptin_period'] ) ? sanitize_text_field( $_POST['uptin_period'] ) : '';
	...
	function refresh_optins_stats_table() {
		wp_verify_nonce( $_POST['uptin_stats_nonce'], 'uptin_stats' );
		$orderby = ! empty( $_POST['uptin_orderby'] ) ? sanitize_text_field( $_POST['uptin_orderby'] ) : '';
		$table   = ! empty( $_POST['uptin_stats_table'] ) ? sanitize_text_field( $_POST['uptin_stats_table'] ) : '';
	...
	function generate_new_account_fields() {
		...
		$service = ! empty( $_POST['uptin_service'] ) ? sanitize_text_field( $_POST['uptin_service'] ) : '';
	...
	function generate_edit_account_page() {
		wp_verify_nonce( $_POST['accounts_tab_nonce'], 'accounts_tab' );
		$edit_account = ! empty( $_POST['uptin_edit_account'] ) ? sanitize_text_field( $_POST['uptin_edit_account'] ) : '';
		$account_name = ! empty( $_POST['uptin_account_name'] ) ? sanitize_text_field( $_POST['uptin_account_name'] ) : '';
		$service      = ! empty( $_POST['uptin_service'] ) ? sanitize_text_field( $_POST['uptin_service'] ) : '';
		...
		printf(
			...
			( 'true' == $edit_account )
				...
				: esc_html__( 'New Account Setup', 'uptin' ),
			( 'true' == $edit_account )
				? esc_html__( 'You can view and re-authorize this account’s settings below', 'uptin' )
				: esc_html__( 'Setup a new email marketing service account below', 'uptin' )
		);
		...
		if ( 'true' == $edit_account ) {
			...
			printf(
					...
					<ul>
						<li class="select rad_dashboard_select_provider_new">
							...
							<select>
								...
								<option value="upviral">%22$s</option>
							</select>
						</li>
					</ul>
				...
				esc_html__( 'New account settings', 'uptin' ),#1
				esc_html__( 'Select One...', 'uptin' ),#2
				esc_html__( 'MailChimp', 'uptin' ),#3
				esc_html__( 'AWeber', 'uptin' ),#4
				esc_html__( 'Constant Contact', 'uptin' ),#5
				esc_html__( 'Campaign Monitor', 'uptin' ),#6
				esc_html__( 'Mad Mimi', 'uptin' ),#7
				esc_html__( 'iContact', 'uptin' ),#8
				esc_html__( 'GetResponse', 'uptin' ),#9
				esc_html__( 'Sendinblue', 'uptin' ),#10
				esc_html__( 'MailPoet', 'uptin' ),#11
				esc_html__( 'save & exit', 'uptin' ),#12
				esc_html__( 'Ontraport', 'uptin' ),#13
				esc_html__( 'Feedblitz', 'uptin' ),#14
				esc_html__( 'Infusionsoft', 'uptin' ),#15
				esc_html__( 'Emma', 'uptin' ),#16
				esc_html__( 'HubSpot Lists', 'uptin' ),#17
				esc_html__( 'Salesforce', 'uptin' ),#18
				esc_html__( 'Active Campaign', 'uptin' ),#19
				esc_html__( 'HubSpot Standard', 'uptin'),#20
				esc_html__( 'Redirect Button', 'uptin'),#21
				//added by mark
				esc_html__( 'UpViral', 'uptin')#22
	...
	function generate_current_lists() {
		wp_verify_nonce( $_POST['accounts_tab_nonce'], 'accounts_tab' );
		$service = ! empty( $_POST['uptin_service'] ) ? sanitize_text_field( $_POST['uptin_service'] ) : '';
		$name    = ! empty( $_POST['uptin_upd_name'] ) ? sanitize_text_field( $_POST['uptin_upd_name'] ) : '';
	...
	function display_currrent_lists( $service = '', $name = '' ) {
		...
		$output = sprintf(
			...
			esc_html__( 'Account Lists', 'uptin' ),
			! empty( $all_lists )
				? implode( ', ', array_map( 'esc_html', $all_lists ) )
				: __( 'No lists available for this account', 'uptin' )
		);
	...
	function save_account_tab() {
		wp_verify_nonce( $_POST['accounts_tab_nonce'], 'accounts_tab' );
		$service = ! empty( $_POST['uptin_service'] ) ? sanitize_text_field( $_POST['uptin_service'] ) : '';
		$name    = ! empty( $_POST['uptin_account_name'] ) ? sanitize_text_field( $_POST['uptin_account_name'] ) : '';
					...
					foreach ( $details as $account_name => $value ) {
						...
						$output .= sprintf( '
							...
							( isset( $value['is_authorized'] ) && 'true' == $value['is_authorized'] )
								...
								esc_attr__( 'Update Lists', 'uptin' ),
								esc_attr( $account_name ),
								esc_attr( $service )
							)
								: '',
							__( 'Remove account', 'uptin' ),
							sprintf(
								...
								esc_html__( 'Remove this account from list?', 'uptin' ),
								esc_html__( 'Yes', 'uptin' ),
								esc_html__( 'No', 'uptin' ),
								esc_attr( $account_name )
							), //#5
							( isset( $value['is_authorized'] ) && 'true' == $value['is_authorized'] )
								? esc_html__( 'Authorized', 'uptin' )
								: esc_html__( 'Not Authorized', 'uptin' ),
							( isset( $value['is_authorized'] ) && 'true' == $value['is_authorized'] )
								? 'check'
								: 'dot',
							esc_html__( 'Edit account', 'uptin' )
						);
						...
						if ( isset( $value['lists'] ) && ! empty( $value['lists'] ) ) {
							foreach ( $value['lists'] as $id => $list ) {
								$output .= sprintf( '
									...
									esc_html( $list['name'] ),
									'ontraport' == $service ? esc_html__( 'n/a', 'uptin' ) : esc_html( $list['subscribers_count'] ),
							...
							$output .= sprintf(
								...
								esc_html__( 'No lists available', 'uptin' )
							);
						}
			...
			if ( isset( $value['optin_status'] ) && $status === $value['optin_status'] && empty( $value['child_of'] ) ) {
				...
				if ( 0 === $optins_count ) {
					...
					$output .= sprintf(
						...
						esc_html__( 'Opt-In Form', 'uptin' ),
						esc_html__( 'Views', 'uptin' ),
						esc_html__( 'Opt-Ins', 'uptin' ),
						esc_html__( 'Conversion Rate', 'uptin' )
					);
				}
				...
				if ( ! empty( $value['child_optins'] ) && 'active' == $status ) {
					...
					foreach ( $child_optins_data as $child_details ) {
						$child_row .= sprintf(
							...
							sprintf(
								...
								esc_html__( 'Delete this optin?', 'uptin' ),
								esc_html__( 'Yes', 'uptin' ),
								esc_html__( 'No', 'uptin' ),
								esc_attr( $child_details['id'] ),
								esc_attr( $optin_id )
							),
							esc_attr__( 'Edit Optin', 'uptin' ),
							esc_attr( $optin_id ) // #9
						);
					}
					...
					$child_row .= sprintf(
						...
						esc_html__( 'Add variant', 'uptin' ),
						( isset( $value['test_status'] ) && 'active' == $value['test_status'] ) ? esc_html__( 'Pause test', 'uptin' ) : esc_html__( 'Start test', 'uptin' ),
						esc_html__( 'End & pick winner', 'uptin' ),
						esc_attr( $optin_id ),
				...
				$output .= sprintf(
					...
					( 'locked' === $value['optin_type'] || 'inline' === $value['optin_type'] || $value['click_trigger'] == '1'  )
						...
						esc_attr__( 'Generate shortcode', 'uptin' ),
						esc_attr( $value['optin_type'] ),
						$value['click_trigger'] == '1' ? 'true' : 'false'
					)
						: '',
					'active' === $status ? esc_html__( 'Make Inactive', 'uptin' ) : esc_html__( 'Make Active', 'uptin' ),
					esc_attr__( 'Duplicate', 'uptin' ),
					esc_attr__( 'Delete Optin', 'uptin' ),
					esc_attr__( 'Edit Optin', 'uptin' ), //#10
		...
		if ( 0 < $optins_count ) {
			if ( 'inactive' === $status ) {
				printf( '
					...
					esc_html__( 'Inactive Opt-Ins', 'uptin' )
				);
			}
	...
	function add_admin_body_class( $classes ) {
		return "$classes rad_uptin";
	}

	function register_scripts( $hook ) {

		wp_enqueue_style( 'rad-uptin-menu-icon', EWD_UPTIN_PLUGIN_URI . '/css/uptin-menu.css', array(), $this->plugin_version );
		...
		wp_enqueue_script('uptin-chart-base', '//www.google.com/jsapi', array(), $this->plugin_version, true );
		wp_enqueue_script('uptin-chart-js', EWD_UPTIN_PLUGIN_URI . '/js/chart.js', array( ), $this->plugin_version, true );
		wp_enqueue_script( 'rad_uptin-uniform-js', EWD_UPTIN_PLUGIN_URI . '/js/jquery.uniform.min.js', array( 'jquery' ), $this->plugin_version, true );
		...
		wp_enqueue_style( 'rad-uptin-css', EWD_UPTIN_PLUGIN_URI . '/css/admin.css', array(), $this->plugin_version );
		wp_enqueue_style( 'rad_uptin-preview-css', EWD_UPTIN_PLUGIN_URI . '/css/style.css', array(), $this->plugin_version );
		wp_enqueue_script( 'rad-uptin-js', EWD_UPTIN_PLUGIN_URI . '/js/admin.js', array( 'jquery', 'uptin-chart-base-init' ), $this->plugin_version, true );
		wp_localize_script( 'rad-uptin-js', 'uptin_settings', array(
			'uptin_nonce'         => wp_create_nonce( 'uptin_nonce' ),
			...
			'optin_type_title'         => __( 'select optin type to begin', 'uptin' ),
			'shortcode_text'           => __( 'Shortcode for this optin:', 'uptin' ),
			...
			'uptin_stats'         => wp_create_nonce( 'uptin_stats_nonce' ),
			'redirect_url'             => rawurlencode( admin_url( 'admin.php?page=' . $this->_options_pagename, $this->protocol ) ),
			'authorize_text'           => __( 'Authorize', 'uptin' ),
			'reauthorize_text'         => __( 'Re-Authorize', 'uptin' ),
			'no_account_name_text'     => __( 'Account name is not defined', 'uptin' ),
			'ab_test_pause_text'       => __( 'Pause test', 'uptin' ),
			'ab_test_start_text'       => __( 'Start test', 'uptin' ),
			'uptin_premade_nonce' => wp_create_nonce( 'uptin_premade' ),
			'preview_nonce'            => wp_create_nonce( 'uptin_preview' ),
			'no_account_text'          => __( 'You Have Not Added An Email List. Before your opt-in can be activated, you must first add an account and select an email list. You can save and exit, but the opt-in will remain inactive until an account is added.', 'uptin' ),
			'add_account_button'       => __( 'Add An Account', 'uptin' ),
			'save_inactive_button'     => __( 'Save As Inactive', 'uptin' ),
			'cannot_activate_text'     => __( 'You Have Not Added An Email List. Before your opt-in can be activated, you must first add an account and select an email list.', 'uptin' ),
			'save_settings'            => wp_create_nonce( 'save_settings' ),
			'chart_stats'              => $this->get_conversions(),
		) );
		wp_enqueue_script('uptin-chart-base-init', EWD_UPTIN_PLUGIN_URI . '/js/googlechart.js', array(), $this->plugin_version, true );
	}
	 ...
	 * @return string
	 */
	function uptin_reset_options_page() {
		...
		EWD_Uptin::generate_options_page( $optin_id );
	...
	function ab_test_actions() {
		...
		EWD_Uptin::update_option( $update_test_status );
	...
	function generate_end_test_modal( $parent_id ) {
		...
		if ( ! empty( $test_optins ) ) {
			...
			$output = sprintf(
				...
				esc_html__( 'Choose an optin', 'uptin' ),
				esc_html__( 'cancel', 'uptin' ),
				$table
			);
		}
	...
	function pick_winner_optin() {
		...
		EWD_Uptin::update_option( $updated_array );
	}
	...
	function perform_option_duplicate( $duplicate_optin_id, $duplicate_optin_type = '', $is_child = false ) {
		...
		if ( '' !== $duplicate_optin_id ) {
			...
			if ( true == $is_child ) {
				...
				EWD_Uptin::update_option( $updated_optin );
			} else {
			...
			EWD_Uptin::update_option( $new_option );
			...
			if ( 'true' == $is_account ) {
				if ( '' !== $service ) {
					if ( isset( $options_array['accounts'][ $service ][ $optin_id ] ) ) {
						...
						EWD_Uptin::update_option( $options_array );
					}
				}
			} else {
				if ( '' != $parent_id ) {
					...
					if ( ! empty( $options_array[ $optin_id ]['child_optins'] ) && true == $remove_child ) {
						foreach ( $options_array[ $optin_id ]['child_optins'] as $single_optin ) {
							EWD_Uptin::remove_option( $single_optin );
							$this->remove_optin_from_db( $single_optin );
						}
					}
				}

				EWD_Uptin::remove_option( $optin_id );
				$this->remove_optin_from_db( $optin_id );
			}
		}
	}
	...
	function toggle_optin_status() {
		...
		if ( '' !== $optin_id ) {
			...
			EWD_Uptin::update_option( $update_option );
		}
	...
	function add_new_account() {
		...
		if ( '' !== $service && '' !== $name ) {
			...
			EWD_Uptin::update_option( $new_account );
		}
	}
			...
			foreach ( $options_array['accounts'] as $service => $account ) {
				foreach ( $account as $name => $details ) {
					if ( 'true' == $details['is_authorized'] ) {
						...
						switch ( $service ) {
							...
							case 'salesforce' :
								...
								break;
							case 'activecampaign':
								$activecampaign = new uptin_activecampaign();
					...
					$result = 'success' == $error_message
						? ''
						: 'uptin_error: ' . $service . ' ' . $name . ' ' . __( 'Authorization failed: ', 'uptin' ) . $error_message;
	...
	function authorize_account() {
		...
		$service         = ! empty( $_POST['uptin_upd_service'] ) ? sanitize_text_field( $_POST['uptin_upd_service'] ) : '';
		$name            = ! empty( $_POST['uptin_upd_name'] ) ? sanitize_text_field( $_POST['uptin_upd_name'] ) : '';
		$update_existing = ! empty( $_POST['uptin_account_exists'] ) ? sanitize_text_field( $_POST['uptin_account_exists'] ) : '';
		...
		if ( 'true' == $update_existing ) {
			...
			$api_key     = ! empty( $_POST['uptin_api_key'] ) ? sanitize_text_field( $_POST['uptin_api_key'] ) : '';
			$token       = ! empty( $_POST['uptin_constant_token'] ) ? sanitize_text_field( $_POST['uptin_constant_token'] ) : '';
			$app_id      = ! empty( $_POST['uptin_client_id'] ) ? sanitize_text_field( $_POST['uptin_client_id'] ) : '';
			$username    = ! empty( $_POST['uptin_username'] ) ? sanitize_text_field( $_POST['uptin_username'] ) : '';
			$password    = ! empty( $_POST['uptin_password'] ) ? sanitize_text_field( $_POST['uptin_password'] ) : '';
			$account_id  = ! empty( $_POST['uptin_username'] ) ? sanitize_text_field( $_POST['uptin_username'] ) : '';
			$public_key  = ! empty( $_POST['uptin_api_key'] ) ? sanitize_text_field( $_POST['uptin_api_key'] ) : '';
			$private_key = ! empty( $_POST['uptin_client_id'] ) ? sanitize_text_field( $_POST['uptin_client_id'] ) : '';
			//start salesforce
			$url = ! empty( $_POST['uptin_url'] ) ? sanitize_text_field( $_POST['uptin_url'] ) : '';
			$version = ! empty( $_POST['uptin_version'] ) ? sanitize_text_field( $_POST['uptin_version'] ) : '';
			$client_key = ! empty( $_POST['uptin_client_key'] ) ? sanitize_text_field( $_POST['uptin_client_key'] ) : '';
			$client_secret = ! empty( $_POST['uptin_client_secret'] ) ? sanitize_text_field( $_POST['uptin_client_secret'] ) : '';
			$username_sf = ! empty( $_POST['uptin_username_sf'] ) ? sanitize_text_field( $_POST['uptin_username_sf'] ) : '';
			$password_sf = ! empty( $_POST['uptin_password_sf'] ) ? sanitize_text_field( $_POST['uptin_password_sf'] ) : '';
			$token = ! empty( $_POST['uptin_token'] ) ? sanitize_text_field( $_POST['uptin_token'] ) : '';
			//end salesforce
		...
		switch ( $service ) {
			...
			//added by markramos
			case 'upviral':
				$upviral = new uptin_upviral();
				$error_message = $upviral->get_upviral_list($app_id, $api_key, $name);
				break;
		...
		$result = 'success' == $error_message ?
			$error_message
			: __( 'Authorization failed: ', 'uptin' ) . $error_message;
	...
	function subscribe() {
		...
		//added by markramos
		$ref_link = sanitize_text_field( $subscribe_data_array['ref_link'] );
		...
		if ( is_email( $email ) ) {
			...
			switch ( $service ) {
				...
				case 'upviral' :
					...
					$upviral = new uptin_upviral();
					if(empty($ref_link)){
						$error_message = $upviral->subscribe_upviral( $api_key, $app_id, $list_id, $email, $name, $last_name );	
					}else{
						$error_message = $upviral->subscribe_upviral_ref( $api_key, $app_id, $list_id, $email, $name, $last_name, $ref_link );
					}
			...
			$error_message = __( 'Invalid email', 'uptin' );
		}

		//added by markramos
		if($service == 'upviral'){
			$error_message = json_decode($error_message);
			if('success' == $error_message->success){
				EWD_Uptin::add_stats_record( 'con', $optin_id, $page_id, $service . '_' . $list_id );
				$result = json_encode( array( 'success' => $error_message->success, 'uId' => $error_message->uId ) );	
			}else{
	...
	function generate_accounts_list() {
		...
		$service     = ! empty( $_POST['uptin_service'] ) ? sanitize_text_field( $_POST['uptin_service'] ) : '';
		$optin_id    = ! empty( $_POST['uptin_optin_id'] ) ? sanitize_text_field( $_POST['uptin_optin_id'] ) : '';
		$new_account = ! empty( $_POST['uptin_add_account'] ) ? sanitize_text_field( $_POST['uptin_add_account'] ) : '';
		...
		if ( ! empty( $available_accounts ) && '' === $new_account ) {
			printf(
				...
				__( 'Select Account', 'uptin' ),
				__( 'Select One...', 'uptin' ),
				selected( 'empty', $current_account, false ),
				esc_attr( $service ),
				__( 'Add Account', 'uptin' )
			);
	...
	function generate_new_account_form( $service, $account_name = '', $display_name = true ) {
		...
		if ( true === $display_name ) {
			$form_fields .= sprintf( '
				...
				__( 'Account Name', 'uptin' ),
				esc_attr( $account_name ),
				EWD_Uptin::generate_hint( __( 'Enter the name for your account', 'uptin' ), true )
			);
		}
		...
		switch ( $service ) {
			...
			case 'emma':
				...
				break;
			case 'salesforce' :
				$salesforce_form = new uptin_salesforce();
			...
			case 'upviral'	:
				$upviral = new uptin_upviral();
				$form_fields = $upviral->draw_upviral_form($form_fields, $service, $field_values);
				break;
		}
	...
	function generate_mailing_lists( $service = '', $account_name = '' ) {
		wp_verify_nonce( $_POST['retrieve_lists_nonce'], 'retrieve_lists' );
		$account_for = ! empty( $_POST['uptin_account_name'] ) ? sanitize_text_field( $_POST['uptin_account_name'] ) : '';
		$service     = ! empty( $_POST['uptin_service'] ) ? sanitize_text_field( $_POST['uptin_service'] ) : '';
		$optin_id    = ! empty( $_POST['uptin_optin_id'] ) ? sanitize_text_field( $_POST['uptin_optin_id'] ) : '';
	...
	function load_scripts_styles() {
		wp_enqueue_script( 'rad_uptin-uniform-js', EWD_UPTIN_PLUGIN_URI . '/js/jquery.uniform.min.js', array( 'jquery' ), $this->plugin_version, true );
		wp_enqueue_script( 'rad_uptin-custom-js', EWD_UPTIN_PLUGIN_URI . '/js/custom.js', array( 'jquery' ), $this->plugin_version, true );
		wp_enqueue_script( 'rad_uptin-idle-timer-js', EWD_UPTIN_PLUGIN_URI . '/js/idle-timer.min.js', array( 'jquery' ), $this->plugin_version, true );
		wp_enqueue_style( 'rad_uptin-open-sans', esc_url_raw( "{$this->protocol}://fonts.googleapis.com/css?family=Open+Sans:400,700" ), array(), null );
		wp_enqueue_style( 'rad_uptin-css', EWD_UPTIN_PLUGIN_URI . '/css/style.css', array(), $this->plugin_version );
		wp_localize_script( 'rad_uptin-custom-js', 'uptinSettings', array(
			'ajaxurl'         => admin_url( 'admin-ajax.php', $this->protocol ),
	...
	/**
	 * Generates the array of all taxonomies supported by Uptin.
	 * Uptin fully supports only taxonomies from ET themes.
	 * @return array
	 */
	...
	public static function choose_form_ab_test( $optin_id, $optins_set, $update_option = true ) {
		...
		if ( ! empty( $optins_set[ $optin_id ]['child_optins'] ) && 'active' == $optins_set[ $optin_id ]['test_status'] ) {
			...
			if ( true === $update_option ) {
				...
				EWD_Uptin::update_uptin_options( $update_test_optin );
			}
		}
	...
	function handle_stats_adding() {
		...
		EWD_Uptin::add_stats_record( $stats_data_array['type'], $stats_data_array['optin_id'], $stats_data_array['page_id'], $stats_data_array['list_id'] );
	...
	function trigger_bottom_mark( $content ) {
		$content .= '<span class="rad_uptin_bottom_trigger"></span>';
	...
	public static function generate_form_content( $optin_id, $page_id, $pagename = '',  $details = array() ) {
		if ( empty( $details ) ) {
			$all_optins = EWD_Uptin::get_uptin_options();
		...
		$hide_img_mobile_class = isset( $details['hide_mobile'] ) && '1' == $details['hide_mobile'] ? 'rad_uptin_hide_mobile' : '';
		$image_animation_class = isset( $details['image_animation'] )
			? esc_attr( ' rad_uptin_image_' . $details['image_animation'] )
			: 'rad_uptin_image_no_animation';
		$image_class           = $hide_img_mobile_class . $image_animation_class . ' rad_uptin_image';

		// Translate all strings if WPML is enabled
		if ( function_exists( 'icl_translate' ) ) {
			$optin_title      = icl_translate( 'uptin', 'optin_title_' . $optin_id, $details['optin_title'] );
			$optin_message    = icl_translate( 'uptin', 'optin_message_' . $optin_id, $details['optin_message'] );
			$email_text       = icl_translate( 'uptin', 'email_text_' . $optin_id, $details['email_text'] );
			$first_name_text  = icl_translate( 'uptin', 'name_text_' . $optin_id, $details['name_text'] );
			$single_name_text = icl_translate( 'uptin', 'single_name_text_' . $optin_id, $details['single_name_text'] );
			$last_name_text   = icl_translate( 'uptin', 'last_name_' . $optin_id, $details['last_name'] );
			$button_text      = icl_translate( 'uptin', 'button_text_' . $optin_id, $details['button_text'] );
			$success_text     = icl_translate( 'uptin', 'success_message_' . $optin_id, $details['success_message'] );
			$footer_text      = icl_translate( 'uptin', 'footer_text_' . $optin_id, $details['footer_text'] );
		} else {
		...
		//added by markramos
		if($details['email_provider'] == 'upviral'){

			// $output .= $details['client_id'];
			$all_optins_uv_main = EWD_Uptin::get_uptin_options();
			// print_r($all_optins_uv_main);

			require_once( EWD_UPTIN_PLUGIN_DIR .'subscription/upviral/upviral.class.php');
			$UpClass = new Upviral($all_optins_uv_main['accounts'][$details['email_provider']][$details['account_name']]['api_key']);
			$js_snippet = $UpClass->getJsSnippetName($details['email_list']);

			$markfname = explode("-",$all_optins_uv_main['accounts'][$details['email_provider']][$details['account_name']]['client_id']);
			$markfrmName = $markfname[count($markfname)-1];
			$markfrmName = 'name="upviralForm'.$markfrmName.'"';
			// $upvir_jscript = "<script language='javascript' src='https://upviral.s3.amazonaws.com/uvjs/".$all_optins_uv_main['accounts'][$details['email_provider']][$details['account_name']]['client_id'].".js'></script>";
			$upvir_jscript = "<script language='javascript' src=".stripcslashes($js_snippet)."></script>";
			$refmark = '<input type="hidden" name="reflink" id="rad_uptin_reflink" value="">';//'.$_GET['ref_id'].'
		}
		...
		$formatted_footer  = '' != $details['footer_text']
			? sprintf(
				'<div class="rad_uptin_form_footer">
					<p>%1$s</p>
				</div>',
		...
		$output = sprintf( '
			<div class="rad_uptin_form_container_wrapper clearfix">
				<div class="rad_uptin_header_outer">
					<div class="rad_uptin_form_header%1$s%13$s">
						%2$s
						%3$s
						%4$s
					</div>
				</div>
				<div class="rad_uptin_form_content%5$s%6$s%7$s%12$s"%11$s>
					%8$s
					<div class="rad_uptin_success_container">
						<span class="rad_uptin_success_checkmark"></span>
					</div>
					<h2 class="rad_uptin_success_message">%9$s</h2>
					%10$s
				</div>
			</div>
			<span class="rad_uptin_close_button"></span>',
			...
			( ( 'above' == $details['image_orientation'] || 'right' == $details['image_orientation'] || 'left' == $details['image_orientation'] ) && 'widget' !== $details['optin_type'] ) || ( 'above' == $details['image_orientation_widget'] && 'widget' == $details['optin_type'] )
				...
				empty( $details['image_url']['id'] )
					...
					: wp_get_attachment_image( $details['image_url']['id'], 'uptin_image', false, array( 'class' => $image_class ) )
			)
				: '',
			( '' !== $formatted_title || '' !== $formatted_message )
				? sprintf(
				'<div class="rad_uptin_form_text">
						%1$s%2$s
					</div>',
			...
			( 'below' == $details['image_orientation'] && 'widget' !== $details['optin_type'] ) || ( isset( $details['image_orientation_widget'] ) && 'below' == $details['image_orientation_widget'] && 'widget' == $details['optin_type'] )
				...
				: '', //#5
			( 'no_name' == $details['name_fields'] && ! EWD_Uptin::is_only_name_support( $details['email_provider'] ) ) || ( EWD_Uptin::is_only_name_support( $details['email_provider'] ) && $is_single_name )
				? ' rad_uptin_1_field'
				: sprintf(
				' rad_uptin_%1$s_fields',
				'first_last_name' == $details['name_fields'] && ! EWD_Uptin::is_only_name_support( $details['email_provider'] )
					? '3'
					: '2'
			),
			'inline' == $details['field_orientation'] && 'bottom' == $details['form_orientation'] && 'widget' !== $details['optin_type']
				? ' rad_uptin_bottom_inline'
				: '',
			( 'stacked' == $details['field_orientation'] && 'bottom' == $details['form_orientation'] ) || 'widget' == $details['optin_type']
				? ' rad_uptin_bottom_stacked'
				: '',
			'custom_html' == $details['email_provider']
				...
				: sprintf( '
					%1$s
					'.$upvir_jscript.'
					<form method="post" '.$markfrmName.' class="clearfix">
						'.$refmark.'
						%3$s
						<p class="rad_uptin_popup_input rad_uptin_subscribe_email %16$s">
						...
						<button data-optin_id="%4$s" data-service="%5$s" data-list_id="%6$s" data-page_id="%7$s" data-post_name="%12$s" data-cookie="%13$s" data-account="%8$s" data-disable_dbl_optin="%11$s" data-redirect_url="%15$s%17$s" data-redirect="%19$s" data-success_delay="%18$s" class="%14$s%22$s" %20$s>
							<span class="rad_uptin_subscribe_loader"></span>
							<span class="rad_uptin_button_text rad_uptin_button_text_color_%10$s">%9$s</span>
						</button>
				...
				'basic_edge' == $details['edge_style'] || '' == $details['edge_style']
					? ''
					: EWD_Uptin::get_the_edge_code( $details['edge_style'], 'widget' == $details['optin_type'] ? 'bottom' : $details['form_orientation'] ),
				'' != $email_text ? stripslashes( esc_attr( $email_text ) ) : esc_html__( 'Email', 'uptin' ),
				( 'no_name' == $details['name_fields'] && ! EWD_Uptin::is_only_name_support( $details['email_provider'] ) ) || ( EWD_Uptin::is_only_name_support( $details['email_provider'] ) && $is_single_name )
					? ''
					: sprintf(
					'<p class="rad_uptin_popup_input rad_uptin_subscribe_name">
					...
					'first_last_name' == $details['name_fields']
						...
						'' != $first_name_text
							...
							: esc_html__( 'First Name', 'uptin' )
					)
						: '',
					( 'first_last_name' != $details['name_fields'] )
						? sprintf( '%1$s', '' != $single_name_text
						? stripslashes( esc_attr( $single_name_text ) )
						: esc_html__( 'Name', 'uptin' ) ) : '',
					'first_last_name' == $details['name_fields'] && ! EWD_Uptin::is_only_name_support( $details['email_provider'] )
						? sprintf( '
									<p class="rad_uptin_popup_input rad_uptin_subscribe_last">
										<input placeholder="%1$s" maxlength="50">
									</p>',
						'' != $last_name_text ? stripslashes( esc_attr( $last_name_text ) ) : esc_html__( 'Last Name', 'uptin' )
					)
						: ''
				),
				esc_attr( $optin_id ),
				...
				'' != $button_text ? stripslashes( esc_html( $button_text ) ) : esc_html__( 'SUBSCRIBE!', 'uptin' ),
				...
				(isset($details['enable_redirect_form']) && $details['enable_redirect_form'] == true)? 'rad_uptin_redirect_page' : 'rad_uptin_submit_subscription',#14
		  ...
		  '' != $success_text
			...
			: esc_html__( 'You have Successfully Subscribed!', 'uptin' ), //#10
			$formatted_footer,
			'custom_html' == $details['email_provider']
				? sprintf(
				...
				: '',
			'custom_html' == $details['email_provider'] ? ' rad_uptin_custom_html_form' : '',
			isset( $details['header_text_color'] )
				? sprintf(
				' rad_uptin_header_text_%1$s',
				esc_attr( $details['header_text_color'] )
			)
				: ' rad_uptin_header_text_dark' //#14
		);

		//added by markramos
		// if($details['email_provider'] == 'upviral'){
		// 	// $output .= $details['client_id'];
		// 	$all_optins_uv = EWD_Uptin::get_uptin_options();
		// 	//print_r($all_optins_uv[$details['account_name']]);
		// 	$uv_name =  $details['account_name'];
		// 	// print_r();	
		// 	$output .= "<script language='javascript' src='https://upviral.s3.amazonaws.com/uvjs/".$all_optins_uv['accounts'][$details['email_provider']][$details['account_name']]['client_id'].".js'></script>";	
		// } 
	...
	public static function get_the_edge_code( $style, $orientation ) {
		...
		switch ( $style ) {
			case 'wedge_edge' :
				$output = sprintf(
					'<svg class="triangle rad_uptin_default_edge" xmlns="http://www.w3.org/2000/svg" version="1.1" width="%2$s" height="%3$s" viewBox="0 0 100 100" preserveAspectRatio="none">
				...
				if ( 'bottom' !== $orientation ) {
					$output .= sprintf(
						'<svg class="triangle rad_uptin_responsive_edge" xmlns="http://www.w3.org/2000/svg" version="1.1" width="%2$s" height="%3$s" viewBox="0 0 100 100" preserveAspectRatio="none">
							<path d="%1$s" fill=""></path>
						</svg>',
			...
			case 'curve_edge' :
				$output = sprintf(
					'<svg class="curve rad_uptin_default_edge" xmlns="http://www.w3.org/2000/svg" version="1.1" width="%2$s" height="%3$s" viewBox="0 0 100 100" preserveAspectRatio="none">
						<path d="%1$s"></path>
					</svg>',
				...
				if ( 'bottom' !== $orientation ) {
					$output .= sprintf(
						'<svg class="curve rad_uptin_responsive_edge" xmlns="http://www.w3.org/2000/svg" version="1.1" width="%2$s" height="%3$s" viewBox="0 0 100 100" preserveAspectRatio="none">
							<path d="%1$s"></path>
						</svg>',
	...
	static function get_power_button( $mode ) {
		...
		// 				<a href="http://www.uptin.com?utm_campaign=rp-rp&utm_medium=powered-by-badge" target="_blank">Powered by<span class="rad_power_logo">&nbsp</span><span class="rad_power_text">Uptin</span></a>
		// 			</span>
		// 		</div>';
		//added by markramos
		return '<div class="rad_power rad_power_mode_' . $mode . '">
							<span class="rad_power_box_mode_' . $mode . '">
								<a href="http://www.uptin.com?utm_campaign=rp-rp&utm_medium=powered-by-badge" target="_blank">Powered by &nbsp;<img src="'.EWD_UPTIN_PLUGIN_URI.'/images/logo.png"></a>
							</span>
						</div>';
	}
		...
		if ( ! empty( $optins_set ) ) {
			foreach ( $optins_set as $optin_id => $details ) {
				if ( $this->check_applicability( $optin_id ) ) {
					$display_optin_id = EWD_Uptin::choose_form_ab_test( $optin_id, $optins_set );
					...
					printf(
						'<div class="rad_uptin_flyin rad_uptin_optin rad_uptin_resize rad_uptin_flyin_%6$s rad_uptin_%5$s%17$s%1$s%2$s%18$s%19$s%20$s%21$s%29$s"%22$s%3$s%4$s%16$s%28$s>
							<div class="rad_uptin_form_container%7$s%8$s%9$s%10$s%12$s%13$s%14$s%15$s%23$s%24$s%25$s">
						...
						true == $details['post_bottom'] ? ' rad_uptin_trigger_bottom' : '',
						isset( $details['trigger_idle'] ) && true == $details['trigger_idle'] ? ' rad_uptin_trigger_idle' : '',
						...
						'bottom' !== $details['form_orientation'] && 'custom_html' !== $details['email_provider']
							...
							: ' rad_uptin_form_bottom',
						'basic_edge' == $details['edge_style'] || '' == $details['edge_style']
							? ''
							: sprintf( ' with_edge %1$s', esc_attr( $details['edge_style'] ) ),
						( 'no_border' !== $details['border_orientation'] )
							? sprintf(
							' rad_uptin_with_border rad_uptin_border_%1$s%2$s',
							esc_attr( $details['border_style'] ),
							esc_attr( ' rad_uptin_border_position_' . $details['border_orientation'] )
						)
							: '',
						( 'rounded' == $details['corner_style'] ) ? ' rad_uptin_rounded_corners' : '', //#10
						EWD_Uptin::generate_form_content( $optin_id, $page_id, $post_name, $details ),
						...
						'stacked' == $details['field_orientation'] && 'bottom' == $details['form_orientation'] && ( 'right' == $details['image_orientation'] || 'left' == $details['image_orientation'] )
							? ' rad_uptin_flyin_bottom_stacked'
							: '', //#27
						EWD_Uptin::get_power_button( 'flyin' ),
						true == $details['click_trigger']
							? ' data-click_trigger="' . esc_attr( $details['click_trigger_selector'] ) . '"'
							: '',#28
						isset( $details['click_trigger'] ) && true == $details['click_trigger'] ? ' rad_uptin_click_trigger' : ''#29
					);
				}
			}
		}
		...
		if ( ! empty( $optins_set ) ) {
			foreach ( $optins_set as $optin_id => $details ) {
				if ( $this->check_applicability( $optin_id ) ) {
					...
					printf(
						'<div class="rad_uptin_popup rad_uptin_optin rad_uptin_resize rad_uptin_%5$s%15$s%21$s%1$s%2$s%16$s%17$s%18$s%20$s%23$s"%3$s%4$s%14$s%19$s%23$s>
							<div class="rad_uptin_form_container rad_uptin_popup_container%6$s%7$s%8$s%9$s%11$s%12$s%13$s">
								%10$s
								%22$s
							</div>
						</div>',
						...
						esc_attr( $optin_id ), // #5
						'bottom' !== $details['form_orientation'] && 'custom_html' !== $details['email_provider']
							? sprintf( ' rad_uptin_form_%1$s', esc_attr( $details['form_orientation'] ) )
							: ' rad_uptin_form_bottom',
						'basic_edge' == $details['edge_style'] || '' == $details['edge_style']
							? ''
							: sprintf( ' with_edge %1$s', esc_attr( $details['edge_style'] ) ),
						( 'no_border' !== $details['border_orientation'] )
							...
							: '',
						( 'rounded' == $details['corner_style'] ) ? ' rad_uptin_rounded_corners' : '',
						EWD_Uptin::generate_form_content( $optin_id, $page_id, $post_name, $details ), //#10
						( 'rounded' == $details['field_corner'] ) ? ' rad_uptin_rounded' : '',
						'light' == $details['text_color'] ? ' rad_uptin_form_text_light' : ' rad_uptin_form_text_dark',
						isset( $details['load_animation'] )
							? sprintf( ' rad_uptin_animation_%1$s', esc_attr( $details['load_animation'] ) )
							: ' rad_uptin_animation_no_animation',
						isset( $details['trigger_idle'] ) && true == $details['trigger_idle']
							? sprintf( ' data-idle_timeout="%1$s"', esc_attr( $details['idle_timeout'] ) )
							: '',
						isset( $details['trigger_auto'] ) && true == $details['trigger_auto'] ? ' rad_uptin_auto_popup' : '', //#15
						isset( $details['comment_trigger'] ) && true == $details['comment_trigger'] ? ' rad_uptin_after_comment' : '',
						isset( $details['purchase_trigger'] ) && true == $details['purchase_trigger'] ? ' rad_uptin_after_purchase' : '',
						isset( $details['trigger_scroll'] ) && true == $details['trigger_scroll'] ? ' rad_uptin_scroll' : '',
						isset( $details['trigger_scroll'] ) && true == $details['trigger_scroll']
							? sprintf( ' data-scroll_pos="%1$s"', esc_attr( $details['scroll_pos'] ) )
							: '',
						( isset( $details['hide_mobile_optin'] ) && true == $details['hide_mobile_optin'] )
						...
						EWD_Uptin::get_power_button( 'popup' ),
						isset( $details['click_trigger'] ) && true == $details['click_trigger'] ? ' rad_uptin_click_trigger' : ''
					);
				}
			}
		}
	...
	function display_preview() {
		wp_verify_nonce( $_POST['uptin_preview_nonce'], 'uptin_preview' );
		...
		if($details['optin_type'] != 'rapidbar') {
			...
			$popup_array['popup_css'] = '<style id="rad_uptin_preview_css">' . EWD_Uptin::generate_custom_css('.rad_uptin .rad_uptin_preview_popup', $details) . '</style>';
		}else{
			$popup_array['popup_code'] = uptin_rapidbar::generate_preview_popup( $details );
			$popup_array['popup_css'] = '<style id="rad_uptin_preview_css">' . uptin_rapidbar::generate_custom_css('.rad_uptin .rad_uptin_preview_rapidbar', $details) . '</style>';
		}
	...
	function generate_preview_popup( $details ) {
		$output = '';
		$output = sprintf(
			'<div class="rad_uptin_popup rad_uptin_animated rad_uptin_preview_popup rad_uptin_optin">
				<div class="rad_uptin_form_container rad_uptin_animation_fadein rad_uptin_popup_container%1$s%2$s%3$s%4$s%5$s%6$s">
					%7$s
					%8$s
				</div>
			</div>',
			...
			( 'rounded' == $details['field_corner'] ) ? ' rad_uptin_rounded' : '',
			'light' == $details['text_color'] ? ' rad_uptin_form_text_light' : ' rad_uptin_form_text_dark',
			EWD_Uptin::generate_form_content( 0, 0, '', $details ),
			EWD_Uptin::get_power_button( 'popup' )
		);
	...
	function display_below_post( $content ) {
		...
		if ( ! empty( $optins_set ) && ! is_singular( 'product' ) ) {
			foreach ( $optins_set as $optin_id => $details ) {
				if ( $this->check_applicability( $optin_id ) ) {
					$content .= '<div class="rad_uptin_below_post">' . $this->generate_inline_form( $optin_id, $details ) . '</div>';
				}
			}
		}
	...
	function display_rapidbar( ) {
		...
		if ( ! empty( $optins_set ) ) {
			foreach ( $optins_set as $optin_id => $details ) {
				if ( $this->check_applicability( $optin_id ) ) {
					$displayCookie = 'rad_uptin_subscribed_to_'.$optin_id.$details['email_list'];
					if(!isset($_COOKIE[$displayCookie])){
						$content = sprintf(
							'<div class="rad_uptin_rapidbar %1$s%3$s %4$s" %2$s>'. $this->generate_rapidbar_form( $optin_id, $details ) . '
							%5$s
							</div>',
							isset( $details['trigger_auto'] ) && true == $details['trigger_auto'] ? 'rad_uptin_rapidbar_trigger_auto' : '',#1
							isset( $details['trigger_auto'] ) && true == $details['trigger_auto']
								? sprintf( 'data-delay="%1$s"', esc_attr( $details['load_delay'] ) )
								: '',#2
							( 'no_border' !== $details['border_orientation'] )
								? sprintf(
								' rad_uptin_border_%1$s%2$s',
								esc_attr( $details['border_style'] ),
								'full' !== $details['border_orientation']
									? ' rad_uptin_border_position_' . $details['border_orientation']
									: ''
							)
								: '',#3
	...
	function generate_rapidbar_form( $optin_id, $details, $update_stats = true ) {
		...
		$all_optins       = EWD_Uptin::get_uptin_options();
		$display_optin_id = EWD_Uptin::choose_form_ab_test( $optin_id, $all_optins );
		...
		if ( true === $update_stats ) {
			EWD_Uptin::add_stats_record( 'imp', $optin_id, $page_id, $list_id );
		}

			$custom_css        = uptin_rapidbar::generate_custom_css( '.rad_uptin .rad_uptin_' . $display_optin_id, $details );
		...
		$output .= sprintf(
			'<div class="rad_uptin_rapidbar_form rad_uptin_optin rad_uptin_%1$s%9$s">
				%10$s
				<div class="rad_uptin_form_container rad_uptin_rapidbar_container%3$s%5$s%6$s%7$s%8$s%11$s">
					%2$s
				</div>

			</div>',
			esc_attr( $optin_id ),
			uptin_rapidbar::generate_form_content( $optin_id, $page_id ),
			'basic_edge' == $details['edge_style'] || '' == $details['edge_style']
				? ''
				: sprintf( ' with_edge %1$s', esc_attr( $details['edge_style'] ) ),
			( 'no_border' !== $details['border_orientation'] )
				...
				: '',
			( 'rounded' == $details['corner_style'] ) ? ' rad_uptin_rounded_corners' : '', //#5
			( 'rounded' == $details['field_corner'] ) ? ' rad_uptin_rounded' : '',
			'light' == $details['text_color'] ? ' rad_uptin_form_text_light' : ' rad_uptin_form_text_dark',
			'bottom' !== $details['form_orientation'] && 'custom_html' !== $details['email_provider']
				? sprintf(
			...
			( 'no_name' == $details['name_fields'] && ! EWD_Uptin::is_only_name_support( $details['email_provider'] ) ) || ( EWD_Uptin::is_only_name_support( $details['email_provider'] ) && $is_single_name )
				? ' rad_uptin_inline_1_field'
				: sprintf(
				' rad_uptin_inline_%1$s_fields',
				'first_last_name' == $details['name_fields'] && ! EWD_Uptin::is_only_name_support( $details['email_provider'] )
					? '3'
					: '2'
			)
	...
	function generate_inline_form( $optin_id, $details, $update_stats = true ) {
		...
		if ( 'below_post' !== $details['optin_type'] ) {
			$custom_css        = EWD_Uptin::generate_custom_css( '.rad_uptin .rad_uptin_' . $display_optin_id, $details );
			$custom_css_output = '' !== $custom_css ? sprintf( '<style type="text/css">%1$s</style>', $custom_css ) : '';
		}

		$output .= sprintf(
			'<div class="rad_uptin_inline_form rad_uptin_optin rad_uptin_%1$s%9$s">
				%10$s
				<div class="rad_uptin_form_container rad_uptin_popup_container%3$s%4$s%5$s%6$s%7$s%8$s%11$s">
					%2$s
				</div>
				%12$s
			</div>',
			esc_attr( $optin_id ),
			EWD_Uptin::generate_form_content( $optin_id, $page_id ),
			'basic_edge' == $details['edge_style'] || '' == $details['edge_style']
				? ''
				: sprintf( ' with_edge %1$s', esc_attr( $details['edge_style'] ) ),
			( 'no_border' !== $details['border_orientation'] )
			...
			EWD_Uptin::get_power_button( 'inline' )
		);
	...
	function display_inline_shortcode( $atts ) {
		...
		$optins_set     = EWD_Uptin::get_uptin_options();
	...
	function display_locked_shortcode( $atts, $content = null ) {
		...
		if ( '' == $selected_optin ) {
			...
			$output = sprintf(
				'<div class="rad_uptin_locked_container rad_uptin_%4$s" data-page_id="%3$s" data-optin_id="%4$s" data-list_id="%5$s">
					<div class="rad_uptin_locked_content" style="display: none;">
						%1$s
					</div>
					<div class="rad_uptin_locked_form">
						%2$s
					</div>
				</div>',
				$content,
	...
	function register_widget() {
		require_once( EWD_UPTIN_PLUGIN_DIR . 'includes/uptin-widget.php' );
		register_widget( 'UptinWidget' );
	}
	...
	public static function display_widget( $optin_id ) {
		...
		if ( '' !== $selected_optin && 'active' == $optins_set[ $optin_id ]['optin_status'] && empty( $optins_set[ $optin_id ]['child_of'] ) ) {
			...
			$custom_css        = EWD_Uptin::generate_custom_css( '.rad_uptin .rad_uptin_' . $display_optin_id, $selected_optin );
			$custom_css_output = '' !== $custom_css ? sprintf( '<style type="text/css">%1$s</style>', $custom_css ) : '';

			EWD_Uptin::add_stats_record( 'imp', $optin_id, $page_id, $list_id );

			$output = sprintf(
				'<div class="rad_uptin_widget_content rad_uptin_optin rad_uptin_%7$s">
					%8$s
					<div class="rad_uptin_form_container rad_uptin_popup_container%2$s%3$s%4$s%5$s%6$s">
						%1$s
					</div>
					%9$s
				</div>',
				...
				( 'no_border' !== $selected_optin['border_orientation'] )
					...
					'full' !== $selected_optin['border_orientation']
						? ' rad_uptin_border_position_' . $selected_optin['border_orientation']
						: ''
				)
					: '',
				( 'rounded' == $selected_optin['corner_style'] ) ? ' rad_uptin_rounded_corners' : '', //#5
				( 'rounded' == $selected_optin['field_corner'] ) ? ' rad_uptin_rounded' : '',
				'light' == $selected_optin['text_color'] ? ' rad_uptin_form_text_light' : ' rad_uptin_form_text_dark',
				esc_attr( $optin_id ),
				$custom_css_output, //#8
				EWD_Uptin::get_power_button( 'widget' )
			);
		}
	...
	public static function widget_optins_list() {
		...
		if ( ! empty( $optins_set ) ) {
			...
			$output = array(
				'empty' => __( 'No Widget optins created yet', 'uptin' ),
			);
		}
	...
	function set_custom_css() {
		$options_array  = EWD_Uptin::get_uptin_options();
		$custom_css     = '';
		$font_functions = EWD_Uptin::load_fonts_class();
		$fonts_array    = array();
		...
		foreach ( $options_array as $id => $single_optin ) {
			if ( 'accounts' != $id && 'db_version' != $id && isset( $single_optin['optin_type'] ) ) {
				if ( 'inactive' !== $single_optin['optin_status'] ) {
					$current_optin_id = EWD_Uptin::choose_form_ab_test( $id, $options_array, false );
					...
					if ( ( ( 'flyin' == $single_optin['optin_type'] || 'pop_up' == $single_optin['optin_type'] || 'below_post' == $single_optin['optin_type'] ) && $this->check_applicability( $id ) ) && ( isset( $single_optin['custom_css'] ) || isset( $single_optin['form_bg_color'] ) || isset( $single_optin['header_bg_color'] ) || isset( $single_optin['form_button_color'] ) || isset( $single_optin['border_color'] ) ) ) {
						$form_class = '.rad_uptin .rad_uptin_' . $current_optin_id;

						$custom_css .= EWD_Uptin::generate_custom_css( $form_class, $single_optin );
					}
		...
		if ( '' != $custom_css ) {
			printf(
				'<style type="text/css" id="rad-uptin-custom-css">
					%1$s
				</style>',
	...
	public static function generate_custom_css( $form_class, $single_optin = array() ) {
		...
		if ( isset( $single_optin['form_bg_color'] ) && '' !== $single_optin['form_bg_color'] ) {
			$custom_css .= $form_class . ' .rad_uptin_form_content { background-color: ' . $single_optin['form_bg_color'] . ' !important; } ';

			if ( 'zigzag_edge' === $single_optin['edge_style'] ) {
				$custom_css .=
					$form_class . ' .zigzag_edge .rad_uptin_form_content:before { background: linear-gradient(45deg, transparent 33.33%, ' . $single_optin['form_bg_color'] . ' 33.333%, ' . $single_optin['form_bg_color'] . ' 66.66%, transparent 66.66%), linear-gradient(-45deg, transparent 33.33%, ' . $single_optin['form_bg_color'] . ' 33.33%, ' . $single_optin['form_bg_color'] . ' 66.66%, transparent 66.66%) !important; background-size: 20px 40px !important; } ' .
					$form_class . ' .zigzag_edge.rad_uptin_form_right .rad_uptin_form_content:before, ' . $form_class . ' .zigzag_edge.rad_uptin_form_left .rad_uptin_form_content:before { background-size: 40px 20px !important; }
					@media only screen and ( max-width: 767px ) {' .
					$form_class . ' .zigzag_edge.rad_uptin_form_right .rad_uptin_form_content:before, ' . $form_class . ' .zigzag_edge.rad_uptin_form_left .rad_uptin_form_content:before { background: linear-gradient(45deg, transparent 33.33%, ' . $single_optin['form_bg_color'] . ' 33.333%, ' . $single_optin['form_bg_color'] . ' 66.66%, transparent 66.66%), linear-gradient(-45deg, transparent 33.33%, ' . $single_optin['form_bg_color'] . ' 33.33%, ' . $single_optin['form_bg_color'] . ' 66.66%, transparent 66.66%) !important; background-size: 20px 40px !important; } ' .
					'}';
			}
		}

		if ( isset( $single_optin['header_bg_color'] ) && '' !== $single_optin['header_bg_color'] ) {
			$custom_css .= $form_class . ' .rad_uptin_form_container .rad_uptin_form_header { background-color: ' . $single_optin['header_bg_color'] . ' !important; } ';
			...
			switch ( $single_optin['edge_style'] ) {
				...
				case 'carrot_edge' :
					$custom_css .=
						$form_class . ' .carrot_edge .rad_uptin_form_content:before { border-top-color: ' . $single_optin['header_bg_color'] . ' !important; } ' .
						$form_class . ' .carrot_edge.rad_uptin_form_right .rad_uptin_form_content:before, ' . $form_class . ' .carrot_edge.rad_uptin_form_left .rad_uptin_form_content:before { border-top-color: transparent !important; border-left-color: ' . $single_optin['header_bg_color'] . ' !important; }
						@media only screen and ( max-width: 767px ) {' .
						$form_class . ' .carrot_edge.rad_uptin_form_right .rad_uptin_form_content:before, ' . $form_class . ' .carrot_edge.rad_uptin_form_left .rad_uptin_form_content:before { border-top-color: ' . $single_optin['header_bg_color'] . ' !important; border-left-color: transparent !important; }
						}';
					break;
			}
			...
			if ( 'dashed' === $single_optin['border_style'] ) {
				if ( 'breakout_edge' !== $single_optin['edge_style'] ) {
					$custom_css .= $form_class . ' .rad_uptin_form_container { background-color: ' . $single_optin['header_bg_color'] . ' !important; } ';
				} else {
					$custom_css .= $form_class . ' .rad_uptin_header_outer { background-color: ' . $single_optin['header_bg_color'] . ' !important; } ';
				}
			}
		}

		if ( isset( $single_optin['form_button_color'] ) && '' !== $single_optin['form_button_color'] ) {
			$custom_css .= $form_class . ' .rad_uptin_form_content button { background-color: ' . $single_optin['form_button_color'] . ' !important; } ';
		}
		...
		if ( isset( $single_optin['border_color'] ) && '' !== $single_optin['border_color'] && 'no_border' !== $single_optin['border_orientation'] ) {
			if ( 'breakout_edge' === $single_optin['edge_style'] ) {
				switch ( $single_optin['border_style'] ) {
					case 'letter' :
						$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_letter .rad_uptin_header_outer { background: repeating-linear-gradient( 135deg, ' . $single_optin['border_color'] . ', ' . $single_optin['border_color'] . ' 10px, #fff 10px, #fff 20px, #f84d3b 20px, #f84d3b 30px, #fff 30px, #fff 40px ) !important; } ';
						break;

					case 'double' :
						$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_double .rad_uptin_form_header { -moz-box-shadow: inset 0 0 0 6px ' . $single_optin['header_bg_color'] . ', inset 0 0 0 8px ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 0 0 6px ' . $single_optin['header_bg_color'] . ', inset 0 0 0 8px ' . $single_optin['border_color'] . '; box-shadow: inset 0 0 0 6px ' . $single_optin['header_bg_color'] . ', inset 0 0 0 8px ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';

						switch ( $single_optin['border_orientation'] ) {
							case 'top' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_double.rad_uptin_border_position_top .rad_uptin_form_header { -moz-box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
								break;

							case 'right' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_double.rad_uptin_border_position_right .rad_uptin_form_header { -moz-box-shadow: inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
								break;

							case 'bottom' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_double.rad_uptin_border_position_bottom .rad_uptin_form_header { -moz-box-shadow: inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
								break;

							case 'left' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_double.rad_uptin_border_position_left .rad_uptin_form_header { -moz-box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
								break;

							case 'top_bottom' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_double.rad_uptin_border_position_top_bottom .rad_uptin_form_header { -moz-box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . ', inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . ', inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . ', inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
								break;

							case 'left_right' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_double.rad_uptin_border_position_left_right .rad_uptin_form_header { -moz-box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . ', inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . ', inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . ', inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
						}
						break;

					case 'inset' :
						$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_inset .rad_uptin_form_header { -moz-box-shadow: inset 0 0 0 3px ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 0 0 3px ' . $single_optin['border_color'] . '; box-shadow: inset 0 0 0 3px ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';

						switch ( $single_optin['border_orientation'] ) {
							case 'top' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_inset.rad_uptin_border_position_top .rad_uptin_form_header { -moz-box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
								break;

							case 'right' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_inset.rad_uptin_border_position_right .rad_uptin_form_header { -moz-box-shadow: inset -3px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset -3px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset -3px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
								break;

							case 'bottom' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_inset.rad_uptin_border_position_bottom .rad_uptin_form_header { -moz-box-shadow: inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
								break;

							case 'left' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_inset.rad_uptin_border_position_left .rad_uptin_form_header { -moz-box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
								break;

							case 'top_bottom' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_inset.rad_uptin_border_position_top_bottom .rad_uptin_form_header { -moz-box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . ', inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . ', inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . ', inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
								break;

							case 'left_right' :
								$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_inset.rad_uptin_border_position_left_right .rad_uptin_form_header { -moz-box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . ', inset -3px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . ', inset -3px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . ', inset -3px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
						}
						break;

					case 'solid' :
						$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_solid .rad_uptin_form_header { border-color: ' . $single_optin['border_color'] . ' !important } ';
						break;

					case 'dashed' :
						$custom_css .= $form_class . ' .breakout_edge.rad_uptin_border_dashed .rad_uptin_form_header { border-color: ' . $single_optin['border_color'] . ' !important } ';
						break;
				}
			} else {
				switch ( $single_optin['border_style'] ) {
					case 'letter' :
						$custom_css .= $form_class . ' .rad_uptin_border_letter { background: repeating-linear-gradient( 135deg, ' . $single_optin['border_color'] . ', ' . $single_optin['border_color'] . ' 10px, #fff 10px, #fff 20px, #f84d3b 20px, #f84d3b 30px, #fff 30px, #fff 40px ) !important; } ';
						break;

					case 'double' :
						$custom_css .= $form_class . ' .rad_uptin_border_double { -moz-box-shadow: inset 0 0 0 6px ' . $single_optin['header_bg_color'] . ', inset 0 0 0 8px ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 0 0 6px ' . $single_optin['header_bg_color'] . ', inset 0 0 0 8px ' . $single_optin['border_color'] . '; box-shadow: inset 0 0 0 6px ' . $single_optin['header_bg_color'] . ', inset 0 0 0 8px ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';

						switch ( $single_optin['border_orientation'] ) {
							case 'top' :
								$custom_css .= $form_class . ' .rad_uptin_border_double.rad_uptin_border_position_top { -moz-box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
								break;

							case 'right' :
								$custom_css .= $form_class . ' .rad_uptin_border_double.rad_uptin_border_position_right { -moz-box-shadow: inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
								break;

							case 'bottom' :
								$custom_css .= $form_class . ' .rad_uptin_border_double.rad_uptin_border_position_bottom { -moz-box-shadow: inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
								break;

							case 'left' :
								$custom_css .= $form_class . ' .rad_uptin_border_double.rad_uptin_border_position_left { -moz-box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
								break;

							case 'top_bottom' :
								$custom_css .= $form_class . ' .rad_uptin_border_double.rad_uptin_border_position_top_bottom { -moz-box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . ', inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . ', inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 8px 0 0 ' . $single_optin['border_color'] . ', inset 0 -6px 0 0 ' . $single_optin['header_bg_color'] . ', inset 0 -8px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
								break;

							case 'left_right' :
								$custom_css .= $form_class . ' .rad_uptin_border_double.rad_uptin_border_position_left_right { -moz-box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . ', inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . ', inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset 8px 0 0 0 ' . $single_optin['border_color'] . ', inset -6px 0 0 0 ' . $single_optin['header_bg_color'] . ', inset -8px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['border_color'] . '; } ';
						}
						break;

					case 'inset' :
						$custom_css .= $form_class . ' .rad_uptin_border_inset { -moz-box-shadow: inset 0 0 0 3px ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 0 0 3px ' . $single_optin['border_color'] . '; box-shadow: inset 0 0 0 3px ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';

						switch ( $single_optin['border_orientation'] ) {
							case 'top' :
								$custom_css .= $form_class . ' .rad_uptin_border_inset.rad_uptin_border_position_top { -moz-box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
								break;

							case 'right' :
								$custom_css .= $form_class . ' .rad_uptin_border_inset.rad_uptin_border_position_right { -moz-box-shadow: inset -3px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset -3px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset -3px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
								break;

							case 'bottom' :
								$custom_css .= $form_class . ' .rad_uptin_border_inset.rad_uptin_border_position_bottom { -moz-box-shadow: inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
								break;

							case 'left' :
								$custom_css .= $form_class . ' .rad_uptin_border_inset.rad_uptin_border_position_left { -moz-box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
								break;

							case 'top_bottom' :
								$custom_css .= $form_class . ' .rad_uptin_border_inset.rad_uptin_border_position_top_bottom { -moz-box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . ', inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . ', inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 0 3px 0 0 ' . $single_optin['border_color'] . ', inset 0 -3px 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
								break;

							case 'left_right' :
								$custom_css .= $form_class . ' .rad_uptin_border_inset.rad_uptin_border_position_left_right { -moz-box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . ', inset -3px 0 0 0 ' . $single_optin['border_color'] . '; -webkit-box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . ', inset -3px 0 0 0 ' . $single_optin['border_color'] . '; box-shadow: inset 3px 0 0 0 ' . $single_optin['border_color'] . ', inset -3px 0 0 0 ' . $single_optin['border_color'] . '; border-color: ' . $single_optin['header_bg_color'] . '; } ';
						}
						break;

					case 'solid' :
						$custom_css .= $form_class . ' .rad_uptin_border_solid { border-color: ' . $single_optin['border_color'] . ' !important } ';
						break;

					case 'dashed' :
						$custom_css .= $form_class . ' .rad_uptin_border_dashed .rad_uptin_form_container_wrapper { border-color: ' . $single_optin['border_color'] . ' !important } ';
						break;
				}
			}
		}

		$custom_css .= isset( $single_optin['form_button_color'] ) && '' !== $single_optin['form_button_color'] ? $form_class . ' .rad_uptin_form_content button { background-color: ' . $single_optin['form_button_color'] . ' !important; } ' : '';
	...
	function add_purchase_trigger() {
		echo '<div class="rad_uptin_after_order"></div>';
	}
	...
	function rad_add_footer_text( $text ) {
		...
		return sprintf( __( $text . ' Uptin - by Emarky<sup>&reg;</sup> <a target="_blank" style= "color:#939AAA;" href="%s">Privacy Policy</a> | <a target="_blank" style= "color:#939AAA;" href="%s">Terms of Use</a>' ), $this->privacy_url, $this->tou_url );
	}

	function execute_footer_text() {
		if ( isset( $_GET['page'] ) ) {
			if ( $_GET['page'] == 'rad_uptin_options' && isset( $_GET['page'] ) ) {
	...
	public function get_error_message( $theme_request, $response_code, $message_map ) {
		...
		if ( is_wp_error( $theme_request ) ) {
			...
			switch ( $response_code ) {
				case '401' :
					$error_message = __( $message_map['401'], 'uptin' );
