...
class ICP_Utils {
    ...
    function query($query, $args=NULL) {
        ...
        if (!is_array($result) || count($result) == 0) {
            ...
            if ($q) {
                foreach ($q as $v) {
                    $result[]=array('id'=>$v->$id, 'text'=>$v->$name);
                }
            } elseif ($query == ICP_QUERY_POST_TYPES) {
        ...
        if ($args['all']) {
            $first=array();
            $first[]=array('id'=>-1, 'name'=>'['.$icp->Lang->L('All').']', 'url'=>'');
            $result=array_merge($first, $result);
        }
        if ($args['select']) {
            $first=array();
            $first[]=array('id'=>0, 'name'=>'['.$icp->Lang->L('Select').']', 'url'=>'');
            $result=array_merge($first, $result);
        }
    ...
    function remotePost($action, $data = '') {
        ...
        $data['secret']='WYSIWYG';
        $response = wp_remote_post(ICP_INTELLYWP_ENDPOINT.'?iwpm_action='.$action, array(
            'method'=>'POST'
            , 'timeout'=>20
            , 'redirection'=>5
            , 'httpversion'=>'1.1'
    ...
    function remoteGet($uri, $options) {
        ...
        /*$args=array('timeout'=>900, 'sslverify'=>false);
            $icp->Log->debug('REMOTEGET: URI=%s', $uri);
            $response=wp_remote_get($uri, $args);
        if (!is_wp_error($response)) {
                // decode the license data
                $body=wp_remote_retrieve_body($response);
            if($body!==FALSE && $body!='') {
                    $icp->Log->debug('REMOTEGET: RETRIEVEBODY=%s', $body);
                    $result=json_decode($body);
                    if(!$result) {
                        $icp->Log->error('REMOTEGET: UNDECODABLE BODY');
                        if(strpos($body, 'debug')!==FALSE || strpos($body, 'fatal')!==FALSE || strpos($body, 'warning')!==FALSE || strpos($body, 'error')!==FALSE) {
                            $icp->Options->pushErrorMessage('XdebugException');
                        }
                    }
                } else {
                    $icp->Log->debug('REMOTEGET: RETRIEVEBODY ERROR');
                    $result=FALSE;
                }
        } else {
            $icp->Log->debug('REMOTEGET: RESULT=ERROR');
        }*/
        ...
        if($result===FALSE) {
            ...
            $icp->Log->debug('file_get_contents: RETRIEVEBODY=%s', $body);
            if($body!==FALSE && $body!='') {
                $icp->Log->debug('REMOTEGET: RETRIEVEBODY=%s', $body);
                $result=json_decode($body);
                if(!$result) {
    ...
    public function mail($to, $subject, $body, $options=array()) {
        ...
        if($options['footer'] && $options['html']) {
            $body .= '<br><hr><a href="'.ICP_WORDPRESS_SITE.'"><img src="'.ICP_SITE_IMAGES_URI.'logos/logo-mail.png" /></a>';
        }
        $result=FALSE;
    ...
    public function cURL($uri, $method, $options=array()) {
        ...
        try {
            ...
            if($errorCode==0 && isset($info['http_code'])) {
                ...
                } elseif(is_string($response) && $icp->Utils->startsWith($response, 'No site configured at this address')) {
                    $errorCode=-1;
                    if($errorDescription=='') {
                        $errorDescription=$response;
                    }
